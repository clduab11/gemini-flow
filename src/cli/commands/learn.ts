...
export class LearnCommand extends Command {
  constructor() {
    ...
    this.description('Learn coding style and patterns from codebase for AI-powered code generation')
      .option('--from <path>', 'Path to learn from (repository or directory)', '.')
      .option('--days <number>', 'Number of days of history to analyze', parseInt, 30)
      .option('--languages <list>', 'Comma-separated list of languages to focus on')
      .option('--patterns <types>', 'Pattern types to learn (architecture,style,naming,testing)', 'architecture,style,naming')
      ...
      .option('--context-window <size>', 'Context window size for analysis', parseInt, 8192)
  ...
  async learnAction(options: any): Promise<void> {
    ...
    try {
      ...
      const stylePatterns = await orchestrator.orchestrate(stylePrompt, {
        ...
        capabilities: ['pattern_recognition', 'style_analysis', 'documentation']
      });
      ...
      if (options.patterns.includes('architecture')) {
        ...
        const archPatterns = await orchestrator.orchestrate(archPrompt, {
          ...
          priority: 'high',
          latencyRequirement: 2000,
          capabilities: ['architecture_analysis', 'pattern_recognition', 'system_design']
        });
      ...
      const compileSpinner = ora('Compiling learned patterns into profile...').start();
      ...
      if (options.saveProfile || options.output) {
        const filename = options.output || `learning-profile-${options.saveProfile || Date.now()}.json`;
      ...
      console.log(chalk.gray(`  • Languages: ${Object.keys(codebaseData.languages).join(', ')}`));
      ...
      console.log(chalk.gray(learningProfile.content.substring(0, 500) + '...'));
  ...
  private async analyzeCodebase(sourcePath: string, options: any): Promise<any> {
    const data = {
      path: sourcePath,
      fileCount: 0,
      languages: {},
    ...
    try {
      // Get file list
      const findOutput = execSync(`find "${sourcePath}" -type f | head -5000`, { encoding: 'utf8' });
      const allFiles = findOutput.trim().split('\n').filter(f => f);
      data.fileCount = allFiles.length;

      // Filter and categorize files
      const codeExtensions = ['.js', '.ts', '.tsx', '.jsx', '.py', '.java', '.cpp', '.c', '.go', '.rs', '.php', '.rb'];
      const testPatterns = ['.test.', '.spec.', '_test.', 'test_', '/tests/', '/test/'];
      const configPatterns = ['package.json', 'requirements.txt', 'Cargo.toml', 'pom.xml', '.env', 'config.'];
      ...
      for (const file of sampleFiles) {
        try {
          ...
          if (content.length < 10000) { // Only sample smaller files
