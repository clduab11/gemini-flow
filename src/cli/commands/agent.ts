...
export class AgentCommand extends Command {
  ...
  private readonly agentTypes: Record<
    string,
  ...
  private createSpawnCommand(): Command {
    ...
    spawn
      ...
      .option(
        "-t, --type <type>",
        'Agent type (use "types" command to see all 64+ types)',
      )
      ...
      .action(async (options) => {
        ...
        try {
          ...
          spinner.succeed(
            chalk.green(`Successfully spawned ${agents.length} agent(s)!`),
          );
  ...
  private createListCommand(): Command {
    ...
    list
      ...
      .option(
        "--sort <field>",
        "Sort by field (name|type|status|performance)",
        "name",
      )
    ...
    stop
      ...
      .action(async (agentId, options) => {
        ...
        try {
          ...
          spinner.succeed(
            chalk.green(`Successfully stopped ${result.count} agent(s)`),
          );

          if ((result.failed as string[]).length > 0) {
            console.log(
              chalk.yellow(
                `Failed to stop: ${(result.failed as string[]).join(", ")}`,
              ),
            );
          }
        } catch (error: any) {
    ...
    restart
      ...
      .action(async (agentId, options) => {
        ...
        try {
          ...
          spinner.succeed(
            chalk.green(`Successfully restarted ${result.count} agent(s)`),
          );

          if ((result.upgraded as string[]).length > 0) {
            console.log(
              chalk.blue(
                `Upgraded: ${(result.upgraded as string[]).join(", ")}`,
              ),
            );
          }
        } catch (error: any) {
    ...
    clone
      ...
      .action(async (sourceAgentId, options) => {
        ...
        try {
          ...
          spinner.succeed(
            chalk.green(`Successfully cloned ${clones.length} agent(s)`),
          );
  ...
  private createUpgradeCommand(): Command {
    ...
    upgrade
      ...
      .argument(
        "[agentId]",
        "Specific agent ID (optional - upgrades all if not specified)",
      )
    ...
    const { agentBatch } = await inquirer.prompt([
      {
        ...
        validate: (input) =>
          input.length > 0 || "Select at least one agent type",
      },
    ]);
  ...
  private async buildSpawnConfig(options: any): Promise<any> {
    if (!options.type) {
      throw new Error(
        "Agent type is required. Use --interactive for guided selection or --type <type>",
      );
    }

    if (!this.agentTypes[options.type]) {
      throw new Error(
        `Unknown agent type: ${options.type}. Use "gemini-flow agent types" to see available types.`,
      );
    }
  ...
  private async initializeAgentCapabilities(
    ...
    for (const agent of agents) {
      this.logger.info(
        `Initializing capabilities for ${agent.type} agent: ${agent.id}`,
      );
  ...
  private displaySpawnedAgents(agents: AgentInfo[]): void {
    ...
    agents.forEach((agent) => {
      ...
      console.log(
        chalk.gray(
          `     Resources: ${agent.resources.memory}MB, ${agent.resources.cpu} CPU`,
        ),
      );
      console.log("");
    });
  ...
  private displayAgentTypes(options: any): void {
    ...
    for (const [category, types] of Object.entries(categories)) {
      ...
      types.forEach((type) => {
        if (this.agentTypes[type]) {
          const info = this.agentTypes[type] as {
            description: string;
            capabilities: string[];
          };
          console.log(
            chalk.white(`  ${type.padEnd(30)} - ${info.description}`),
          );
          console.log(
            chalk.gray(`    Capabilities: ${info.capabilities.join(", ")}`),
          );
        }
      });
      console.log("");
    ...
    console.log(
      chalk.cyan(
        `Total: ${Object.keys(this.agentTypes).length} specialized agent types available`,
      ),
    );
    console.log(
      chalk.gray(
  ...
  private async realTimeAgentMonitoring(
    agentId: string,
    _options: Record<string, unknown>,
  ): Promise<void> {
  ...
  private async showAgentMetrics(
    _agentId: string,
    _options: Record<string, unknown>,
  ): Promise<void> {
  ...
  private async realTimeMetricsDashboard(
    _agentId: string,
    _options: Record<string, unknown>,
  ): Promise<void> {
