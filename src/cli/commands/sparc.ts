...
export class SparcCommand extends Command {
  ...
  private createRunCommand(): Command {
    ...
    run
      ...
      .argument(
        "<mode>",
        'SPARC mode to run (use "modes" command to see available modes)',
      )
      ...
      .action(async (mode, task, options) => {
        ...
        try {
          ...
          if (!this.sparcModes[mode]) {
            throw new Error(
              `Unknown SPARC mode: ${mode}. Use "gemini-flow sparc modes" to see available modes.`,
            );
          }
          ...
          const results = await this.executeSparcPhases(
            swarmId,
            executionPlan,
            options,
          );
          ...
          spinner.succeed(
            chalk.green(`SPARC ${mode} mode completed successfully!`),
          );
    ...
    tdd
      ...
      .action(async (feature, options) => {
        ...
        try {
          ...
          const tddResults = await this.executeTddCycle(
            tddSwarm,
            feature,
            options,
          );
  ...
  private createReportCommand(): Command {
    ...
    report
      ...
      .option(
        "--format <format>",
        "Report format (html|pdf|json|markdown)",
        "html",
      )
  ...
  private async initializeSparcSwarm(
    sparcMode: SparcMode,
    options: any,
  ): Promise<string> {
  ...
  private async setupSparcCoordination(
    swarmId: string,
    sparcMode: SparcMode,
    options: any,
  ): Promise<void> {
  ...
  private async saveSparcProgress(
    swarmId: string,
    results: any,
  ): Promise<void> {
  ...
  private displaySparcResults(results: any): void {
    ...
    results.phases.forEach((phase: any) => {
      ...
      console.log(
        chalk.white(`   ${phase.phase}: ${phase.status} (${duration}s)`),
      );
    });
    ...
    console.log(
      chalk.white(
        `   Total Time: ${(results.metrics.totalTime / 1000).toFixed(2)}s`,
      ),
    );
    console.log(
      chalk.white(`   Test Coverage: ${results.metrics.testCoverage}%`),
    );
    console.log(
      chalk.white(`   Code Quality: ${results.metrics.codeQuality}/10`),
    );
  ...
  private displaySparcModes(options: any): void {
    ...
    console.log(
      chalk.gray('Usage: gemini-flow sparc run <mode> "<task description>"'),
    );
  }
  ...
  private async setupTddEnvironment(
    feature: string,
    options: any,
  ): Promise<void> {
  ...
  private async storeSparcMemory(
    key: string,
    context: string,
    options: any,
  ): Promise<void> {
