...
export function registerHiveCommands(program: Command) {
  ...
  const modelOrchestratorConfig: ModelOrchestratorConfig = {
  ...
  const communicationCoordinatorConfig: CommunicationCoordinatorConfig = { projectID: 'your-gcp-project-id' };
  const communicationCoordinator = new CommunicationCoordinator(communicationCoordinatorConfig);

  // --- Initialize Hive-Mind Core ---
  const queenConfig: QueenConfig = { id: 'queen-001', name: 'HiveQueen', vertexAiModel: 'gemini-pro', firestoreCollection: 'hive_global_state' };
  const queenAgent = new QueenAgent(queenConfig, dbCore, memoryIntelligence, toolExecutor, toolRegistry);

  const consensusConfig: ConsensusConfig = { minParticipants: 3, faultTolerancePercentage: 0.33, timeoutMs: 1000 };
  const byzantineConsensus = new ByzantineConsensus(consensusConfig, dbCore);

  const coordinationEngineConfig: CoordinationEngineConfig = { projectID: 'your-gcp-project-id' };
  ...
  const gcpOperationsSuiteConfig: GcpOperationsSuiteConfig = { projectID: 'your-gcp-project-id' };
  const gcpOperationsSuiteIntegration = new GcpOperationsSuiteIntegration(gcpOperationsSuiteConfig);

  const vertexAiPerformanceOptimizerConfig: VertexAiPerformanceOptimizerConfig = { projectID: 'your-gcp-project-id' };
  ...
  hiveCommand
    ...
    .action(async (workerCount: number) => {
  ...
  hiveCommand
    ...
    .action(async (proposalId: string, proposalValue: string) => {
