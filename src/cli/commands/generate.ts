...
export class GenerateCommand extends Command {
  constructor() {
    ...
    this.description('AI-powered code generation using learned styles and patterns')
      .argument('<description>', 'Description of what to generate')
      .option('--style <profile>', 'Style profile to use (learned, clean, enterprise)', 'learned')
      .option('--framework <name>', 'Target framework (react, express, fastapi, etc.)')
      .option('--language <lang>', 'Target programming language')
      .option('--type <type>', 'Code type (component, api, class, function, test)', 'component')
      .option('--template <name>', 'Base template to use')
      .option('--output-dir <path>', 'Output directory for generated files', './generated')
      .option('--include-tests', 'Generate tests along with code')
      .option('--include-docs', 'Generate documentation')
      .option('--coverage-target <n>', 'Test coverage target percentage', parseInt, 90)
  ...
  async generateAction(description: string, options: any): Promise<void> {
    ...
    try {
      ...
      // Load style profile
      const styleProfile = await this.loadStyleProfile(options.style, options.styleFile);
      
      spinner.succeed('Generation system initialized');

      // Phase 1: Analyze Requirements
      console.log(chalk.blue('\n📋 Phase 1: Requirement Analysis'));
      const analysisSpinner = ora('Analyzing generation requirements...').start();
      ...
      console.log(chalk.gray(`  • Framework: ${options.framework || 'auto-detected'}`));
      console.log(chalk.gray(`  • Tests included: ${options.includeTests ? 'Yes' : 'No'}`));
      console.log(chalk.gray(`  • Documentation: ${options.includeDocs ? 'Yes' : 'No'}`));
      ...
      if (options.interactive) {
        ...
        console.log(chalk.cyan('💡 Use --preview to see code before writing files'));
      }
  ...
  private async loadStyleProfile(style: string, styleFile?: string): Promise<any> {
    try {
  ...
  private async writeGeneratedFiles(content: string, options: any): Promise<void> {
    try {
      ...
      const fileBlocks = content.split('```').filter((block, index) => index % 2 === 1);
