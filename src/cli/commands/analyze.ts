...
export class AnalyzeCommand extends Command {
  ...
  async analyzeAction(options: any): Promise<void> {
  ...
  private analyzeRepositoryStructure(repoPath: string): any {
    try {
      ...
      try {
        const findOutput = execSync(`find "${repoPath}" -type f | head -1000`, { encoding: 'utf8' });
  ...
  private analyzeGitHistory(repoPath: string, period: string): any {
    try {
      ...
      const gitPeriod = period === '24h' ? '1 day' : 
                       period === '7d' ? '1 week' : 
                       period === '30d' ? '1 month' : '1 month';

      try {
        // Get recent commits
        const commitLog = execSync(
          `cd "${repoPath}" && git log --since="${gitPeriod} ago" --pretty=format:"%h|%an|%ad|%s" --date=iso`,
          { encoding: 'utf8' }
        );

        if (commitLog.trim()) {
          stats.commits = commitLog.trim().split('\n').map(line => {
          ...
          stats.commits.forEach(commit => {
            stats.contributors[commit.author] = (stats.contributors[commit.author] || 0) + 1;
          });
        }

        // Get file change statistics
        const fileStats = execSync(
          `cd "${repoPath}" && git log --since="${gitPeriod} ago" --name-only --pretty=format: | sort | uniq -c | sort -nr | head -20`,
          { encoding: 'utf8' }
        );

        if (fileStats.trim()) {
          fileStats.trim().split('\n').forEach(line => {
