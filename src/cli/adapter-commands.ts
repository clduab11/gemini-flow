...
async function testAdapters(options: any): Promise<void> {
  ...
  try {
    ...
    for (const test of testRequests) {
      if (options.model && !test.name.toLowerCase().includes(options.model.toLowerCase())) {
        continue;
      }
 ...
 */
async function testStreaming(manager: AdapterManager, request: ModelRequest): Promise<void> {
  try {
    let chunkCount = 0;
    let totalContent = '';
    const startTime = performance.now();
...
async function checkAdapterHealth(options: any): Promise<void> {
  ...
  try {
    ...
    const checkHealth = async () => {
      ...
      console.log('┌─────────────────────────┬──────────┬──────────┬─────────────┐');
      console.log('│ Adapter                 │ Status   │ Latency  │ Last Check  │');
      console.log('├─────────────────────────┼──────────┼──────────┼─────────────┤');
      
      for (const [name, healthCheck] of Object.entries(health)) {
        const status = getHealthEmoji(healthCheck.status) + ' ' + healthCheck.status.padEnd(8);
        const latency = `${healthCheck.latency.toFixed(0)}ms`.padEnd(8);
        const lastCheck = healthCheck.lastChecked.toLocaleTimeString().padEnd(11);
        
        console.log(`│ ${name.padEnd(23)} │ ${status} │ ${latency} │ ${lastCheck} │`);
      }
...
async function benchmarkAdapters(options: any): Promise<void> {
  ...
  try {
    ...
    if (options.output === 'json') {
      ...
      console.log('┌─────────────────────────┬──────────┬─────────────┬──────────┐');
      console.log('│ Adapter                 │ Latency  │ Success     │ Errors   │');
      console.log('├─────────────────────────┼──────────┼─────────────┼──────────┤');
...
async function testRouting(options: any): Promise<void> {
  ...
  try {
    ...
    console.log('┌─────────────────────────┬─────────────────────┬─────────┬────────────┐');
    console.log('│ Test Name               │ Selected Adapter    │ Score   │ Reasoning  │');
    console.log('├─────────────────────────┼─────────────────────┼─────────┼────────────┤');

    for (const test of testRequests) {
      try {
        const decision = await manager.getRoutingDecision(test.request as ModelRequest);
        ...
        if (options.verbose) {
          console.log(`  └─ Routing time: ${decision.routingTime.toFixed(1)}ms`);
        ...
        console.log(`│ ${name} │ ERROR               │ N/A     │ ${errorMsg} │`);
      }
    }
    ...
    const showMetrics = async () => {
      ...
      if (Object.keys(metrics.modelDistribution).length > 0) {
        ...
        for (const [model, count] of Object.entries(metrics.modelDistribution)) {
          const percentage = ((count as number) / metrics.totalRequests * 100).toFixed(1);
