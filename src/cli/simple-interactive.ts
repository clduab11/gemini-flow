...
export class SimpleInteractive {
      ...
      const model = genAI.getGenerativeModel({
        ...
        generationConfig: {
          ...
          maxOutputTokens: Math.min(
            4096,
            (this.options.maxTokens || 1000000) / 4,
          ),
        },
      });
  ...
  private showHelp(): void {
    ...
    console.log(
      chalk.cyan("/history        "),
      chalk.gray("Show conversation history"),
    );
    console.log(
      chalk.cyan("/model <name>   "),
      chalk.gray("Change model (gemini-1.5-flash, gemini-1.5-pro)"),
    );
    console.log(
      chalk.cyan("/temperature <n>"),
      chalk.gray("Change temperature (0-2)"),
    );
    console.log(chalk.cyan("/save [id]      "), chalk.gray("Save session"));
    console.log(chalk.cyan("/load [id]      "), chalk.gray("Load session"));
    console.log(
      chalk.cyan("/stats          "),
      chalk.gray("Show session statistics"),
    );
    console.log(
      chalk.cyan("/exit           "),
      chalk.gray("Exit interactive mode"),
    );
    console.log();
  }
  ...
  private showHistory(): void {
    ...
    this.history.forEach((entry, _index) => {
      const role =
        entry.role === "user" ? chalk.green("You") : chalk.blue("Assistant");
      const timestamp = entry.timestamp.toLocaleTimeString();
      console.log(
        `${role} [${timestamp}]: ${entry.content.substring(0, 100)}${entry.content.length > 100 ? "..." : ""}`,
      );
    });
  ...
  private async changeModel(modelName: string): Promise<void> {
    if (!modelName) {
      ...
      console.log(
        chalk.cyan("  • gemini-1.5-flash") + chalk.gray(" (fast, versatile)"),
      );
      console.log(
        chalk.cyan("  • gemini-1.5-pro") + chalk.gray(" (complex reasoning)"),
      );
      console.log(
        chalk.cyan("  • gemini-1.0-pro") + chalk.gray(" (text and code)"),
      );
      return;
    }
  ...
  private showStats(): void {
    ...
    const totalTokens = this.history.reduce(
      (sum, entry) => sum + this.estimateTokens(entry.content),
      0,
    );
  ...
  private async loadSession(sessionId?: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        chalk.green(
          `✅ Session loaded: ${id} (${this.history.length} messages)`,
        ),
      );
      this.options.sessionId = id;
    } catch (error) {
