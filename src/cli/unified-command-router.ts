...
export interface CommandResult {
  ...
  error?: string;
  duration: number;
  router: "official" | "fallback";
}

export interface RouterEventMap {
  "command-start": [context: CommandContext];
  "command-success": [result: CommandResult];
  "command-error": [error: Error, context: CommandContext];
  "command-fallback": [context: CommandContext, reason: string];
  "official-cli-detected": [version: string, path: string];
  "official-cli-not-found": [error?: string];
}
...
export class UnifiedCommandRouter extends EventEmitter {
  ...
  private cliDetector: GeminiCLIDetector;
  private activeProcesses: Set<ChildProcess> = new Set();
  private config: {
    enableContextPassing: boolean;
   ...
   */
  async routeCommand(context: CommandContext): Promise<CommandResult> {
    ...
    try {
      ...
      if (this.config.enableDeprecationWarnings) {
        this.logger.warn(
          "Official Gemini CLI not available or failed. " +
   ...
   */
  private async tryOfficialCLI(context: CommandContext): Promise<Omit<CommandResult, "duration" | "router">> {
   ...
   */
  private executeOfficialCLI(
    cliPath: string,
    args: string[],
    context: CommandContext
  ): Promise<Omit<CommandResult, "duration" | "router">> {
   ...
   */
  private async routeToFallback(context: CommandContext): Promise<Omit<CommandResult, "duration" | "router">> {
   ...
   */
  private prepareCommandArgs(context: CommandContext): string[] {
  ...
  getStats(): {
    ...
    totalRoutedCommands: number;
    officialCLIRouteCount: number;
   ...
   */
  emit<K extends keyof RouterEventMap>(
    event: K,
  ...
  on<K extends keyof RouterEventMap>(
    event: K,
    listener: (...args: RouterEventMap[K]) => void
  ): this {
 ...
 */
export async function routeGeminiCommand(context: CommandContext): Promise<CommandResult> {
 ...
 */
export async function shouldUseOfficialCLI(): Promise<boolean> {
 ...
 */
export async function getCurrentRoutingMode(): Promise<"official" | "fallback"> {
