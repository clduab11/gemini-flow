...
describe('AgentSpace Integration Tests', () => {
  ...
  const testConfig: AgentSpaceInitConfig = {
    ...
    initialAgentTypes: ['hierarchical-coordinator', 'coder', 'researcher', 'tester'],
  ...
  describe('System Initialization', () => {
    ...
    it('should handle invalid configuration gracefully', async () => {
      ...
      await expect(initializeAgentSpace(invalidConfig, memoryManager))
        .rejects.toThrow();
    });
    ...
    it('should validate memory manager integration', async () => {
      ...
      const testMemoryNode = {
        ...
        metadata: { source: 'test', accessibility: 'high', relevanceScore: 0.9 },
        relationships: []
      };

      await result.agentSpaceManager.memoryArchitecture.storeMemoryNode(testMemoryNode);
      
      const retrievedNodes = await result.agentSpaceManager.memoryArchitecture
        .queryMemoryBySpatialProximity({ x: 0, y: 0, z: 0 }, 10);
  ...
  describe('Agent Deployment and Management', () => {
    ...
    it('should deploy full agent swarm with optimal spatial arrangement', async () => {
      ...
      for (const agentId of result.deployedAgents) {
        ...
        deployedAgentPositions.push({ x: Math.random() * 100, y: Math.random() * 100, z: 0 });
      }
  ...
  describe('MCP Bridge Integration', () => {
    ...
    it('should spawn agent with spatial integration through MCP', async () => {
      ...
      expect(agentResult.spatialContext.position).toEqual({ x: 25, y: 25, z: 10 });
    });
    ...
    it('should bridge memory operations with spatial context', async () => {
      const memoryResult = await mcpBridge.bridgeMemoryOperationWithSpatialContext(
        'store',
        {
  ...
  describe('Streaming Integration', () => {
    ...
    it('should create spatial streaming session', async () => {
      ...
      expect(sessionResult.session.spatialContext.position).toEqual({ x: 0, y: 0, z: 0 });
    ...
    it('should start spatial video stream with 3D context', async () => {
      ...
      expect(videoResult.videoStream.resolution).toEqual({ width: 1280, height: 720 });
    ...
    it('should start spatial audio stream with 3D processing', async () => {
      ...
      const session = await streamingIntegration.createSpatialStreamingSession({
        ...
        workspaceId: workspace.id,
        sessionType: 'audio',
        spatialConfig: { position: { x: 20, y: 20, z: 10 }, spatialAudioEnabled: true }
      });
  ...
  describe('Performance and Scalability', () => {
    ...
    it('should handle concurrent workspace creation', async () => {
      ...
      const workspacePromises = Array.from({ length: concurrentOperations }, (_, i) =>
    ...
    it('should maintain performance under memory pressure', async () => {
      ...
      const storePromises = Array.from({ length: memoryNodes }, (_, i) =>
        agentSpaceSystem.manager.memoryArchitecture.storeMemoryNode({
          ...
          metadata: { source: 'performance-test', accessibility: 'high', relevanceScore: 0.8 },
          relationships: []
        })
      );
      ...
      // Query memory nodes
      const queryResults = await agentSpaceSystem.manager.memoryArchitecture
        .queryMemoryBySpatialProximity({ x: 5, y: 5, z: 0 }, 20);
    ...
    it('should scale spatial operations efficiently', async () => {
      const agentCount = 25;
      const positions: Vector3D[] = Array.from({ length: agentCount }, (_, i) => ({
        x: (i % 5) * 20,
        y: Math.floor(i / 5) * 20,
        z: 0
      }));
      ...
      const avgNearbyEntities = queryResults.reduce((sum, result) => sum + result.length, 0) / queryResults.length;
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should handle memory operation failures', async () => {
      ...
      const invalidNode = {
        ...
        position: { x: 0, y: 0, z: 0 },
        content: null,
        metadata: { source: 'test', accessibility: 'high', relevanceScore: 0.5 },
        relationships: []
      };
      
      await expect(agentSpaceSystem.manager.memoryArchitecture.storeMemoryNode(invalidNode))
        .rejects.toThrow();
    });
