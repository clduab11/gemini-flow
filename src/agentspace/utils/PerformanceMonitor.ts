...
export class PerformanceMonitor extends EventEmitter {
  ...
  async detectBottlenecks(): Promise<BottleneckAnalysis[]> {
    ...
    if (currentMetrics.resourceUsage.cpu.utilization > this.config.bottleneckDetectionThreshold) {
      bottlenecks.push(this.createBottleneckAnalysis(
        'cpu',
    ...
    if (currentMetrics.resourceUsage.memory.utilization > this.config.bottleneckDetectionThreshold) {
      bottlenecks.push(this.createBottleneckAnalysis(
        'memory',
    ...
    if (currentMetrics.latency.p95Latency > 1000) { // 1 second threshold
      bottlenecks.push(this.createBottleneckAnalysis(
        'algorithm',
  ...
  generateAlerts(): PerformanceAlert[] {
    ...
    if (currentMetrics.latency.p95Latency > 2000) {
      alerts.push(this.createAlert(
        ...
        ['Investigate slow operations', 'Optimize critical paths', 'Scale resources']
      ));
    }
    ...
    if (currentMetrics.throughput.operationsPerSecond < 10) {
      alerts.push(this.createAlert(
        ...
        ['Check for bottlenecks', 'Optimize processing', 'Review resource allocation']
      ));
    }
  ...
  private calculateTrend(values: number[]): { slope: number; prediction: number; confidence: number } {
    if (values.length < 2) {
    ...
    const residualSumSquares = values.reduce(
      (sum, val, index) => sum + Math.pow(val - (intercept + slope * index), 2), 0
    );
  ...
  private getTrendDirection(slope: number): 'improving' | 'stable' | 'degrading' {
  ...
  private getMetricValue(metrics: SystemPerformanceMetrics, metricPath: string): number {
