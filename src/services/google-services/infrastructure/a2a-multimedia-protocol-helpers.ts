...
export class ProtocolHelpers {
  ...
  async establishRoutingPath(sessionId: string, agentId: string): Promise<void> {
    this.logger.debug('Establishing routing path', { sessionId, agentId });
    // Simulate routing path establishment
    await new Promise(resolve => setTimeout(resolve, 10));
  }
  
  async deliverMessage(message: A2AMultimediaMessage, targetId: string): Promise<boolean> {
  ...
  async handleRoutingFailover(message: A2AMultimediaMessage, failedHop: string): Promise<boolean> {
  ...
  async createHttpStreamingEndpoint(streamId: string, targetAgent: string, config: any): Promise<string> {
    this.logger.debug('Creating HTTP streaming endpoint', { streamId, targetAgent });
  ...
  async createMulticastEndpoint(streamId: string, config: any): Promise<string> {
  ...
  createRetryStrategy(maxAttempts: number = 3, baseDelay: number = 1000): {
    attempt: number;
