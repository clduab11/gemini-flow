...
export class GeminiAdapter extends BaseModelAdapter {
  private googleAIClient?: any;
  private isGoogleAIInitialized = false;
   ...
   */
  private async initializeGoogleAIClient(): Promise<void> {
    ...
    try {
      ...
      if (!apiKey) {
        throw this.createError(
          "Google AI API key not found. Please set GOOGLE_API_KEY environment variable or configure apiKey in adapter config.",
          "MISSING_API_KEY",
          401,
          false,
        );
      ...
      throw this.createError(
        `Failed to initialize Google AI client: ${error instanceof Error ? error.message : 'Unknown error'}`,
   ...
   */
  private prepareRequestParts(request: ModelRequest): any[] {
    const parts: any[] = [];
   ...
   */
  private mapSafetyRatings(safetyRatings?: any[]): any {
   ...
   */
  private handleGoogleAIError(error: any, request: ModelRequest): never {
    ...
    switch (errorCode) {
      ...
      case 503:
        throw this.createError(
          `Google AI service error: ${errorMessage}`,
          "SERVICE_UNAVAILABLE",
          errorCode,
          true,
        );
