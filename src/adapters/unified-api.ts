...
export class UnifiedAPI extends EventEmitter {
  ...
  private performanceHistory: Array<{ timestamp: Date; latency: number; adapter: string }> = [];

  // Fast routing optimization
  private routingDecisionCache = new Map<string, { decision: RoutingDecision; timestamp: number }>();
  ...
  async generate(request: ModelRequest): Promise<ModelResponse> {
    ...
    try {
      ...
      const totalLatency = performance.now() - startTime;
      await this.handleRequestError(error as AdapterError, request, totalLatency);
      throw error;
    }
  }
   ...
   */
  private async makeRoutingDecision(request: ModelRequest): Promise<RoutingDecision> {
    ...
    for (const adapterName of candidates) {
      ...
      switch (this.config.routing.strategy) {
        case 'latency':
          ...
          break;
        case 'cost':
          score = factors.cost * 0.6 + factors.latency * 0.3 + factors.availability * 0.1;
          break;
        case 'quality':
          score = factors.capability * 0.5 + factors.latency * 0.3 + factors.availability * 0.2;
          break;
        case 'balanced':
          score = factors.latency * 0.3 + factors.cost * 0.25 + factors.capability * 0.25 + factors.availability * 0.2;
          break;
        default: // custom
   ...
   */
  private async executeWithAdapter(adapterName: string, request: ModelRequest): Promise<ModelResponse> {
    const adapter = this.adapters.get(adapterName);
    if (!adapter) {
  ...
  private extractRequiredCapabilities(request: ModelRequest): string[] {
    ...
    if (request.tools && request.tools.length > 0) capabilities.push('functions');
    ...
    const reasoningKeywords = ['analyze', 'compare', 'evaluate', 'reason', 'think'];
    if (reasoningKeywords.some(kw => request.prompt.toLowerCase().includes(kw))) {
      capabilities.push('reasoning');
    }

    // Detect code requirements
    const codeKeywords = ['code', 'function', 'class', 'programming', 'algorithm'];
   ...
   */
  private calculateLatencyScore(adapterName: string, request: ModelRequest): number {
   ...
   */
  private calculateCostScore(adapterName: string, request: ModelRequest): number {
   ...
   */
  private calculateCapabilityScore(adapterName: string, request: ModelRequest): number {
   ...
   */
  private async selectFallbackAdapter(failedAdapter: string, request: ModelRequest): Promise<string | null> {
    ...
    const capableAdapters = this.filterCapableAdapters(request)
      .filter(name => availableAdapters.includes(name));
  ...
  private async handleRequestError(
    ...
    if (error.retryable && (request.context?.retryCount || 0) < this.config.routing.retryAttempts) {
   ...
   */
  private updateMetrics(adapter: string, latency: number, success: boolean, response?: ModelResponse): void {
    if (success) {
    ...
    this.metrics.modelDistribution[adapter] = (this.metrics.modelDistribution[adapter] || 0) + 1;

    // Update cost metrics
    if (response) {
      this.metrics.costMetrics.totalCost += response.cost;
      this.metrics.costMetrics.costPerRequest = this.metrics.costMetrics.totalCost / this.metrics.totalRequests;
      
      if (response.usage.totalTokens > 0) {
        this.metrics.costMetrics.costPerToken = this.metrics.costMetrics.totalCost / this.getTotalTokensProcessed();
      }
    }
  }
  ...
  private analyzePerformance(): void {
    ...
    this.metrics.performanceMetrics = {
      ...
      throughput: recentHistory.length / ((recentHistory[recentHistory.length - 1].timestamp.getTime() - recentHistory[0].timestamp.getTime()) / 1000)
    };
  ...
  private updateErrorMetrics(error: AdapterError): void {
    this.metrics.errorDistribution[error.code] = (this.metrics.errorDistribution[error.code] || 0) + 1;
  }
  ...
  private calculateCustomScore(factors: any, _request: ModelRequest): number {
    ...
    return factors.latency * 0.4 + factors.capability * 0.3 + factors.availability * 0.2 + factors.cost * 0.1;
  }

  private generateReasoningExplanation(selected: { adapter: string; score: number; factors: any }, _request: ModelRequest): string {
    const dominant = Object.entries(selected.factors)
  ...
  private calculateAverageRoutingTime(): number {
    ...
    return recentDecisions.reduce((sum, time) => sum + time, 0) / recentDecisions.length;
  }
