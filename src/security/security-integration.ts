...
import { getSecurityConfig, zeroTrustPolicy, validateSecurityConfig } from './production-security-config.js';
...
export class SecurityIntegrationManager {
  ...
  async initialize(): Promise<{
    ...
    try {
      ...
      if (!validation.isValid) {
        ...
        return { success: false, components, errors, warnings: validation.warnings };
      }
      ...
      errors.push(error instanceof Error ? error.message : 'Unknown initialization error');
  ...
  async handleSecurityIncident(
    ...
    const incidentId = await this.securityHardening.createSecurityIncident(incident);

    // Determine appropriate runbook
    const runbookId = this.determineRunbook(incident.category, incident.severity);
    ...
    if (runbookId && this.runbooks) {
      try {
        ...
        this.logger.error('Runbook execution failed', { error, runbookId, incidentId });
      }
    }

    // Trigger escalation for high/critical incidents
    const escalationTriggered = incident.severity === 'high' || incident.severity === 'critical';
  ...
  async authorizeAccess(
    ...
    return await this.zeroTrustArch.authorizeAccess(sessionId, resource, action, contextInfo);
  }
   ...
   */
  async executeSecureQuery(query: string, parameters: any[], options?: any): Promise<any> {
    if (!this.securityHardening) {
      throw new Error('Security hardening not initialized');
    }

    return await this.securityHardening.executeParameterizedQuery(query, parameters, options);
  }
  ...
  async generateSecurityDashboard(): Promise<any> {
    ...
    try {
      if (this.securityHardening) {
        const dashboard = await this.securityHardening.generateSecurityDashboard();
    ...
    try {
      ...
      for (const statement of statements) {
        try {
          ...
          this.logger.debug('Schema statement execution', { statement: statement.substring(0, 100), error });
        }
      }
  ...
  private async setupEventHandlers(): Promise<void> {
    ...
    if (this.runbooks && this.securityHardening) {
      ...
      this.runbooks.on('escalation_required', async (execution, step, result) => {
 ...
 */
export async function createSecurityFramework(db: DatabaseConnection): Promise<SecurityIntegrationManager> {
  const manager = new SecurityIntegrationManager(db);
  
  const result = await manager.initialize();
  
  if (!result.success) {
    throw new Error(`Security framework initialization failed: ${result.errors.join(', ')}`);
  }
 ...
 */
export function createSecurityMiddleware(securityManager: SecurityIntegrationManager) {
  return {
    ...
    validateInput: (schema: any) => {
      return async (req: any, res: any, next: any) => {
        try {
          if (req.body) {
            const validation = await securityManager.validateInput(req.body, schema);
            if (!validation.isValid) {
    ...
    csrfProtection: async (req: any, res: any, next: any) => {
      ...
      try {
        const isValid = await securityManager.validateCsrfToken(sessionId, token);
        if (!isValid) {
      ...
      try {
        const credentials = {
          ...
          contextInfo: {
            ...
            location: req.headers['x-location'] ? JSON.parse(req.headers['x-location']) : undefined
          }
        };
