...
export class MemorySharding extends EventEmitter {
  ...
  addNode(nodeId: string, capacity: number): void {
    ...
    if (this.config.strategy === 'consistent_hash' || this.config.strategy === 'hybrid') {
  ...
  async mergeShards(shard1Id: string, shard2Id: string): Promise<string> {
    ...
    const [leftShard, rightShard] = shard1.endKey === shard2.startKey 
      ? [shard1, shard2] 
      : [shard2, shard1];
   ...
   */
  async rebalanceShards(nodes: Array<{ agentId: string; capacity: number }>): Promise<void> {
    const imbalance = this.calculateImbalance();
    
    if (imbalance < this.config.rebalanceThreshold) {
      this.logger.debug('Shards are balanced, no rebalancing needed', { imbalance });
      return;
    }
  ...
  private selectReplicaNodes(primaryNodeId: string): string[] {
    ...
    const availableNodes = Array.from(this.shardMap.nodeAssignments.keys())
  ...
  private createRebalancePlan(nodes: Array<{ agentId: string; capacity: number }>): MigrationTask[] {
  ...
  private updateMetrics(): void {
    ...
    this.metrics.averageShardSize = sizes.length > 0 
    ...
    const totalCapacity = Array.from(this.shardMap.nodeAssignments.keys()).length * this.config.targetShardSize;
    const totalUsed = sizes.reduce((a, b) => a + b, 0);
    this.metrics.storageEfficiency = totalCapacity > 0 ? totalUsed / totalCapacity : 0;
