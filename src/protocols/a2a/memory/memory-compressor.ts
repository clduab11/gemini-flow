...
export class MemoryCompressor extends EventEmitter {
  ...
  constructor(enabled: boolean = true) {
    ...
    this.logger.info('Memory compressor initialized', { enabled: this.enabled });
  }
   ...
   */
  garbageCollect(maxAge: number = 3600000): number { // 1 hour default
    ...
    for (const [hash, result] of this.compressionHistory) {
      ...
      if (Math.random() < 0.1) { // Randomly clean 10%
  ...
  private selectOptimalAlgorithm(fingerprint: DataFingerprint): CompressionAlgorithm {
    // Find matching rule
  ...
  private async compressDelta(data: Buffer, options: CompressionOptions): Promise<Buffer> {
  ...
  private createDeduplicationResult(hash: string, originalSize: number): CompressionResult {
    return {
  ...
  private updateCompressionStats(result: CompressionResult): void {
    ...
    this.stats.averageRatio = 
      (this.stats.averageRatio + result.ratio) / 2;
