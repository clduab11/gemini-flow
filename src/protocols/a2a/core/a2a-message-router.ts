...
export class A2AMessageRouter extends EventEmitter {
  ...
  async updateAgentMetrics(agentId: AgentId, metrics: any): Promise<void> {
    ...
    if (metrics.responseTime !== undefined && routingEntry.agentCard.metadata.metrics) {
      routingEntry.agentCard.metadata.metrics.responseTime = metrics.responseTime;
    }
  ...
  getRoutingMetrics(): RoutingMetrics {
    ...
    return {
      ...
      avgRoutingTime: this.metrics.routingTimes.length > 0
        ? this.metrics.routingTimes.reduce((a, b) => a + b, 0) / this.metrics.routingTimes.length
        : 0,
   ...
   */
  private async routeCapabilityAware(message: A2AMessage): Promise<MessageRoute> {
    ...
    for (const agent of candidateAgents) {
      ...
      if (score.factors.capability > 0) { // Has at least some required capabilities
        scores.push(score);
      }
    }
   ...
   */
  private getCandidateAgents(to: AgentId | AgentId[] | 'broadcast'): AgentCard[] {
    if (to === 'broadcast') {
    ...
    return {
      ...
      factors: {
        ...
        latency: agent.metadata.metrics ? 
          1 / (agent.metadata.metrics.responseTime.avg || 1) : 0.5,
   ...
   */
  private calculateVersionCompatibility(agentVersion: string, requiredVersion: string): number {
    try {
   ...
   */
  private trackRoutingSuccess(strategy: RoutingStrategy, route: MessageRoute, routingTime: number): void {
