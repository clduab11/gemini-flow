...
import { A2AIdentity, A2AMessage, SecurityEvent } from '../../../core/a2a-security-manager.js';
...
export class MaliciousAgentDetector extends EventEmitter {
  ...
  constructor() {
    ...
    this.logger.info('Malicious Agent Detection System initialized', {
      features: [
        'behavioral-analysis', 'ml-anomaly-detection', 'consensus-based-detection',
        'byzantine-fault-tolerance', 'real-time-monitoring', 'automatic-quarantine'
      ],
      config: this.config
    });
  }
    ...
    try {
      ...
      if (anomalies.length > 0) {
        ...
        anomalies.forEach(anomaly => {
          const count = profile.anomalyIndicators.anomalyTypes.get(anomaly.type) || 0;
  ...
  private async performDetectionRound(): Promise<void> {
    try {
      const activeAgents = Array.from(this.behaviorProfiles.keys())
        .filter(agentId => !this.quarantinedAgents.has(agentId));

      for (const agentId of activeAgents) {
        const detectionResult = await this.analyzeAgentBehavior(agentId);
        
        if (detectionResult && detectionResult.confidence > this.config.anomalyThreshold) {
   ...
   */
  private async analyzeAgentBehavior(agentId: string): Promise<MaliciousDetectionResult | null> {
    const profile = this.behaviorProfiles.get(agentId);
    if (!profile) return null;

    const detectionResults: { pattern: string; confidence: number; evidence: any[] }[] = [];

    // Check each detection pattern
    for (const [patternId, pattern] of this.detectionPatterns) {
      const patternResult = await this.evaluateDetectionPattern(profile, pattern);
    ...
    const mlConfidence = mlAnomalies.reduce((sum, a) => sum + a.confidence, 0) / mlAnomalies.length || 0;
    ...
    const overallConfidence = detectionResults.length > 0 ?
      detectionResults.reduce((sum, r) => sum + r.confidence, 0) / detectionResults.length : 0;
  ...
  private async evaluateDetectionRule(
    ...
    switch (rule.operator) {
      case 'gt':
        ...
        confidence = matches ? Math.min(1, (actualValue - rule.threshold) / rule.threshold) : 0;
        break;
      case 'lt':
        matches = actualValue < rule.threshold;
        confidence = matches ? Math.min(1, (rule.threshold - actualValue) / rule.threshold) : 0;
        break;
      case 'eq':
        matches = actualValue === rule.threshold;
   ...
   */
  private extractMetricValue(profile: BehaviorProfile, metricPath: string): any {
   ...
   */
  private async initiateConsensusDetection(detection: MaliciousDetectionResult): Promise<void> {
  ...
  async submitConsensusVote(vote: ConsensusVote): Promise<void> {
    ...
    const existingVote = votes.find(v => v.voterId === vote.voterId && v.round === vote.round);
    if (existingVote) {
  ...
  private async processConsensusVotes(): Promise<void> {
    for (const [targetAgentId, votes] of this.consensusVotes) {
      ...
      const currentRoundVotes = votes.filter(v => v.round === detection.consensusRound);
      ...
      const avgConfidence = maliciousVotes.reduce((sum, v) => sum + v.confidence, 0) / maliciousVotes.length || 0;

      if (consensusRatio >= this.config.consensusThreshold && avgConfidence >= this.config.quarantineThreshold) {
   ...
   */
  private async quarantineAgent(agentId: string, detection: MaliciousDetectionResult): Promise<void> {
   ...
   */
  async processRecoveryResponse(agentId: string, response: any): Promise<boolean> {
    if (!this.quarantinedAgents.has(agentId)) {
      return false;
    }
  ...
  private createInitialBehaviorProfile(agentId: string, identity: A2AIdentity): BehaviorProfile {
    return {
      agentId,
  ...
  private async updateBehaviorProfile(
    ...
    profile.messageFrequency.perMinute = this.updateMovingAverage(
      profile.messageFrequency.perMinute, 1, 60
    );
    
    // Update message patterns
    profile.messagePatterns.avgPayloadSize = this.updateMovingAverage(
      profile.messagePatterns.avgPayloadSize, payloadSize, 100
    );
    
    const typeCount = profile.messagePatterns.messageTypes.get(message.type) || 0;
    ...
    if (profile.messagePatterns.timePatterns.length > 168) { // Keep 1 week
      profile.messagePatterns.timePatterns = profile.messagePatterns.timePatterns.slice(-168);
    }
    ...
    if (metadata.resourceUsage) {
      profile.networkBehavior.resourceUsage = this.updateMovingAverage(
        profile.networkBehavior.resourceUsage, metadata.resourceUsage, 100
      );
    }
  }
  ...
  private async detectRealTimeAnomalies(
    ...
    const hourlyPattern = profile.messagePatterns.timePatterns.filter(h => h === currentHour).length;
  ...
  private determineRecommendedAction(confidence: number): 'monitor' | 'warn' | 'restrict' | 'quarantine' | 'ban' {
  ...
  private async generateRecoveryChallenge(agentId: string): Promise<{ type: string; challenge: any }> {
  ...
  private async validateRecoveryResponse(agentId: string, response: any): Promise<boolean> {
  ...
  async getSystemStats(): Promise<any> {
    return {
      ...
      consensusVotes: Array.from(this.consensusVotes.values()).reduce((sum, votes) => sum + votes.length, 0),
  ...
  private calculateAverageBehaviorScore(): number {
    ...
    const totalScore = profiles.reduce((sum, p) => sum + p.trustMetrics.behaviorScore, 0);
...
class BehaviorModel {
  ...
  async predict(profile: BehaviorProfile): Promise<{ isMalicious: boolean; confidence: number }> {
...
class MLAnomalyDetector {
  ...
  async detectAnomalies(profile: BehaviorProfile): Promise<{ type: string; confidence: number; description: string }[]> {
...
class ConsensusAnalyzer {
  async analyzeConsensusPattern(votes: ConsensusVote[]): Promise<{ isValid: boolean; confidence: number }> {
