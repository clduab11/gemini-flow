...
export class AgentCardSystem extends EventEmitter {
  ...
  async discoverAgents(request: DiscoveryRequest): Promise<DiscoveryResponse> {
    ...
    try {
      ...
      if (
        ...
        candidateAgents = this.filterByCapabilities(
          candidateAgents,
          request.params.capabilities,
        );
      ...
      if (request.params.agentType) {
        candidateAgents = candidateAgents.filter(
          (entry) => entry.agentCard.metadata.type === request.params.agentType,
        );
      }

      // Apply custom filters
      if (request.params.filters && request.params.filters.length > 0) {
        candidateAgents = this.applyFilters(
          candidateAgents,
      ...
      if (request.params.maxDistance !== undefined) {
        candidateAgents = this.filterByDistance(
          candidateAgents,
          request.params.maxDistance,
        );
      }
  ...
  async findAgentsByCapability(
    ...
    for (const agentId of agentIds) {
      ...
      const capability = agentCard.capabilities.find(
  ...
  async getSystemMetrics(): Promise<SystemMetrics> {
    ...
    return {
      ...
      agentsByStatus,
      averageLoad:
        activeAgents.length > 0 ? totalLoad / activeAgents.length : 0,
  ...
  private filterByCapabilities(
    ...
    return candidates.filter((entry) => {
      const agentCapabilities = entry.agentCard.capabilities.map(
        (cap) => cap.name,
      );
  ...
  private applyFilters(
    ...
    return candidates.filter((entry) => {
      return filters.every((filter) =>
        this.evaluateFilter(entry.agentCard, filter),
      );
    });
  }
        ...
        case "contains":
          if (Array.isArray(fieldValue)) {
            return fieldValue.some((item) =>
              this.deepIncludes(item, filter.value),
            );
          }
  ...
  private isVersionCompatible(
    ...
    try {
      ...
      if (
        agentVer.minor === requiredVer.minor &&
