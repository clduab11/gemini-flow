...
expect.extend({
  toBeValidProtocolConfig(received: any) {
    ...
    const validVersion = typeof received.version === 'string' && /^\d+\.\d+\.\d+$/.test(received.version);
  ...
  toMeetTopologyRequirements(received: any, topology: string) {
    const requirements = {
      ...
      hierarchical: (nodes: any[]) => nodes.some(n => n.role === 'coordinator'),
...
expect.extend({
  ...
  toBeSynchronizedWith(received: any, otherState: any) {
    ...
    const isSynchronized = receivedClock && otherClock &&
...
expect.extend({
  ...
  toBeValidConsensusMessage(received: any) {
    const requiredFields = ['type', 'viewNumber', 'sequenceNumber', 'timestamp', 'senderId'];
    const validTypes = ['pre-prepare', 'prepare', 'commit', 'view-change', 'new-view'];
...
expect.extend({
  toMeetPerformanceRequirements(received: any, thresholds: any) {
    ...
    const requirements = [
      { key: 'latency', check: (value: number) => value <= (thresholds.maxLatency || 1000) },
      { key: 'throughput', check: (value: number) => value >= (thresholds.minThroughput || 100) },
      { key: 'memory', check: (value: number) => value <= (thresholds.maxMemory || 100 * 1024 * 1024) },
      { key: 'cpu', check: (value: number) => value <= (thresholds.maxCpu || 80) }
    ];
...
expect.extend({
  ...
  toHaveEventualConsistency(received: any, otherStates: any[], timeoutMs: number = 5000) {
    // Mock implementation for eventual consistency check
    // In real implementation, this would wait for states to converge
    const hasConsistency = otherStates.every(state => 
      JSON.stringify(received) === JSON.stringify(state)
    );
