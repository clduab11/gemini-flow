...
import { ProtocolActivator, ProtocolConfig, ProtocolStatus, ActivationResult, TopologyType } from '../protocol-activator.js';
...
describe('ProtocolActivator', () => {
  ...
  describe('Topology Validation', () => {
    ...
    it('should reject invalid topology values', async () => {
      ...
      await expect(activator.activateProtocol('A2A', invalidTopology))
        .rejects
        .toThrow("Invalid topology 'invalid'. Must be one of: hierarchical, mesh, ring, star");
    });

    it('should reject undefined topology', async () => {
      // @ts-ignore - Testing runtime behavior with invalid input
      await expect(activator.activateProtocol('A2A', undefined))
        .rejects
        .toThrow("Invalid topology 'undefined'. Must be one of: hierarchical, mesh, ring, star");
    });
    ...
    it('should require topology for autoActivate', async () => {
      ...
      await expect(activator.autoActivate(invalidTopology))
        .rejects
        .toThrow("Invalid topology 'invalid'. Must be one of: hierarchical, mesh, ring, star");
    });
  });
  ...
  describe('Protocol Activation', () => {
    ...
    describe('Activation Failures', () => {
      ...
      it('should handle activation errors gracefully', async () => {
        ...
        jest.doMock('../simple-a2a-adapter.js', () => ({
          SimpleA2AAdapter: jest.fn().mockImplementation(() => ({
            initialize: jest.fn().mockRejectedValue(new Error('Fallback failed'))
          }))
        }));
  ...
  describe('Topology Requirements Validation', () => {
    describe('Network Topology Validation', () => {
      it('should validate mesh topology requirements', () => {
        ...
        const isValidMesh = mockTopology.nodes.every(node => 
          node.connections.length === mockTopology.nodes.length - 1
        );
      ...
      it('should validate ring topology requirements', () => {
        ...
        const isValidRing = mockTopology.nodes.every(node => 
          'prev' in node && 'next' in node && node.prev !== node.next
        );
    ...
    describe('Protocol Compatibility Validation', () => {
      it('should validate A2A protocol requirements', () => {
        ...
        const availableCapabilities = ['agent-communication', 'distributed-memory', 'consensus-protocols'];
        const hasRequiredCapabilities = a2aRequirements.requiredCapabilities.every(
    ...
    describe('Resource Requirements Validation', () => {
      ...
      it('should validate port availability', () => {
        ...
        // Mock port checker
        const mockPortChecker = jest.spyOn(activator as any, 'checkPortsAvailable');
