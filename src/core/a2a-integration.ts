...
export class A2ASecurityIntegration extends EventEmitter {
  ...
  private setupEventHandlers(): void {
    ...
    if (this.a2aSecurityManager) {
      this.a2aSecurityManager.on('agent_registered', (identity: A2AIdentity) => {
  ...
  async authenticateEntity(
    ...
    try {
      ...
      } else if (entityType === 'agent' && this.a2aSecurityManager && options.enableA2A !== false) {
  ...
  async checkAccess(
    ...
    try {
      ...
      this.metrics.integrationErrors++;
      this.logger.error('Access check failed', { entityId, resource, action, error });
  ...
  async checkRateLimit(
    ...
    try {
      ...
      this.metrics.integrationErrors++;
      this.logger.error('Rate limit check failed', { entityId, entityType, error });
  ...
  async logSecurityEvent(
    ...
    try {
      ...
      if (this.auditLogger && (entityType === 'agent' || this.config.bridgeToExistingSecurity)) {
      ...
      this.metrics.integrationErrors++;
      this.logger.error('Security event logging failed', { eventType, entityId, error });
    }
  }
    ...
    try {
      ...
      if (entityType === 'agent') {
        ...
        if (this.zeroTrust) {
          ...
          status.quarantined = await this.zeroTrust.isAgentQuarantined(entityId);
        }
        
        if (this.auditLogger) {
          const alerts = this.auditLogger.getSecurityAlerts(10);
          status.securityAlerts = alerts.filter(alert => 
            alert.source.agentId === entityId
          );
        }
      ...
      this.logger.error('Failed to get security status', { entityId, entityType, error });
      return {};
    }
  }
  ...
  async emergencyLockdown(
    ...
    this.logger.error('Emergency security lockdown initiated', { reason, scope });
  ...
  private collectMetrics(): void {
    ...
    this.metrics.systemResourceUtilization = this.calculateResourceUtilization();
  ...
  private async performHealthCheck(): Promise<void> {
    ...
    if (unhealthyComponents.length > 0) {
      this.logger.warn('Unhealthy components detected', { unhealthyComponents });
    }
  }
