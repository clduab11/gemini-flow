...
import { ModelOrchestrator, ModelResponse, RoutingContext } from '../model-orchestrator.js';
...
describe('Smart Routing Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(orchestrator as any, 'executeGeminiRequest').mockResolvedValue(mockGeminiResponse);
  });
  ...
  describe('End-to-End Routing Performance', () => {
    ...
    test('should handle rapid successive requests efficiently', async () => {
      const requests = 10;
      const contexts: RoutingContext[] = Array(requests).fill(null).map((_, i) => ({
  ...
  describe('Performance Monitoring Integration', () => {
    ...
    test('should warn on performance degradation', (done) => {
      ...
      const heavyContext: RoutingContext = {
        ...
        tokenBudget: 8192,
        capabilities: ['code', 'advanced-reasoning', 'enterprise-security', 'multimodal']
      };
  ...
  describe('Failover and Resilience', () => {
    test('should handle model failures gracefully', async () => {
      ...
      jest.spyOn(orchestrator as any, 'executeGeminiRequest')
  ...
  describe('User Tier Compliance', () => {
    test('should respect tier restrictions in routing', async () => {
      const contexts = [
        ...
        { userTier: 'enterprise' as const, expectedModelOption: ['gemini-pro-vertex', 'gemini-2.0-flash-thinking'] }
      ];
  ...
  describe('Health Monitoring', () => {
    ...
    test('should detect unhealthy models', async () => {
      ...
      jest.spyOn(orchestrator as any, 'executeWithModel')
    ...
    test('should maintain consistent performance over time', async () => {
      ...
      for (let batch = 0; batch < batches; batch++) {
        ...
        const promises = Array(batchSize).fill(null).map((_, i) =>
          orchestrator.orchestrate(`Consistency test batch ${batch} item ${i}`, {
