...
export class FeatureFlags {
  ...
  private static readonly DEFAULT_CONFIG: FeatureFlagConfig = {
    ...
    vertexAi: { enabled: false, mode: 'auto', dependencies: ['@google-cloud/vertexai'] },
    googleWorkspace: { enabled: false, mode: 'auto', dependencies: ['googleapis'] },
  ...
  private performRuntimeChecks(): void {
    ...
    this.checkDependency('vertexAi', ['@google-cloud/vertexai', '@google-cloud/aiplatform']);
    this.checkDependency('googleWorkspace', ['googleapis', 'google-auth-library']);
   ...
   */
  private checkDependency(feature: keyof FeatureFlagConfig, deps: string[]): void {
    ...
    if (hasAnyDep) {
      this.logger.debug(`Dependency available for ${feature}:`, deps.find(dep => {
        try { require.resolve(dep); return true; } catch { return false; }
      }));
    }
  }
   ...
   */
  private checkSystemCapability(feature: keyof FeatureFlagConfig, check: () => boolean): void {
    try {
   ...
   */
  private checkNodeCapability(feature: keyof FeatureFlagConfig, check: () => boolean): void {
    try {
      const available = check();
      this.runtimeChecks.set(feature, available);
  ...
  getStatusReport(): any {
    ...
    Object.keys(this.config).forEach(feature => {
      ...
      report.dependencyStatus[feature] = this.runtimeChecks.get(feature) ?? false;
    });
  ...
  saveConfiguration(): void {
    try {
      ...
      const configToSave = Object.fromEntries(
        Object.entries(this.config).filter(([, config]) => config.mode === 'manual')
      );
