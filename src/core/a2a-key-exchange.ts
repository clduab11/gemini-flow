...
export class A2AKeyExchange extends EventEmitter {
   ...
   */
  private initializeRotationPolicy(policy: Partial<KeyRotationPolicy> = {}): void {
    this.rotationPolicy = {
  ...
  private async initializeMasterKeys(): Promise<void> {
    try {
      ...
      const masterKeyInfo: KeyPairInfo = {
        ...
        algorithm: 'ECDH',
        curve: 'secp384r1',
        publicKey: this.masterKeyPair.publicKey.export({ type: 'spki', format: 'pem' }) as string,
        createdAt: new Date(),
  ...
  async generateAgentKeyPair(
    ...
    try {
      ...
      const keyInfo: KeyPairInfo = {
        ...
        privateKey: this.hsmConfig.enabled ? undefined : 
      ...
      this.logger.error('Failed to generate agent key pair', { agentId, error });
      throw error;
    }
  }
    ...
    try {
      ...
      if (accept) {
        ...
        const secretInfo: SharedSecret = {
          ...
          agentPair: [request.fromAgentId, request.toAgentId].sort() as [string, string],
          sharedSecret,
          derivedKeys,
          createdAt: new Date(),
          expiresAt: new Date(Date.now() + this.rotationPolicy.keyLifetime),
   ...
   */
  async getSharedSecret(agentId1: string, agentId2: string): Promise<SharedSecret | null> {
    const agentPair = [agentId1, agentId2].sort() as [string, string];
    
    // Check memory first
    for (const secret of this.sharedSecrets.values()) {
      if (secret.agentPair[0] === agentPair[0] && secret.agentPair[1] === agentPair[1]) {
        // Check if secret is still valid
        if (secret.expiresAt > new Date() && secret.usageCount < secret.maxUsage) {
          return secret;
        }
      }
    }
  ...
  async generateDistributedKeyShares(
    ...
    try {
      ...
      const sharesKey = crypto.createHash('sha256').update(secretData).digest('hex');
   ...
   */
  async reconstructSecretFromShares(shares: DistributedKeyShare[]): Promise<Buffer> {
    try {
  ...
  private async performECDH(publicKeyPem: string, privateKeyPem: string): Promise<Buffer> {
  ...
  private async signKeyExchangeRequest(request: KeyExchangeRequest): Promise<string> {
  ...
  private async signKeyExchangeResponse(response: KeyExchangeResponse): Promise<string> {
    const data = JSON.stringify({
  ...
  private async verifyKeyExchangeRequest(request: KeyExchangeRequest): Promise<boolean> {
    const data = JSON.stringify({
      requestId: request.requestId,
      fromAgentId: request.fromAgentId,
      toAgentId: request.toAgentId,
  ...
  private generateShamirPolynomial(secret: Buffer, threshold: number): Buffer[] {
  ...
  private async generateVerificationProof(share: Buffer, commitment: string): Promise<string> {
    // Generate zero-knowledge proof for share verification
    const proof = crypto.createHmac('sha256', share)
      .update(commitment)
      .digest();
    ...
    for (const [keyId, keyInfo] of this.keyPairs) {
      ...
      const needsRotation = 
        ...
        (this.rotationPolicy.preRotationWarning > 0 && 
         keyInfo.expiresAt.getTime() - now.getTime() <= this.rotationPolicy.preRotationWarning);
      ...
      if (needsRotation && keyId !== 'master') {
        ...
        try {
          ...
          this.logger.error('Scheduled key rotation failed', { keyId, agentId, error });
        }
      }
    }
    ...
    for (const [secretId, secret] of this.sharedSecrets) {
      if (secret.rotationScheduled <= now || 
        ...
        try {
          ...
          this.logger.error('Shared secret rotation failed', { secretId, error });
        }
      }
    }
  }
  ...
  async updateRotationPolicy(updates: Partial<KeyRotationPolicy>): Promise<void> {
    ...
    if (agentId) {
      ...
      for (const agentId of activeAgents) {
        try {
          ...
          this.logger.error('Emergency rotation failed for agent', { agentId, error });
        }
      }
    }
