...
export class ToolRegistry {
  ...
  private toolsMetadata: Map<string, RegisteredToolMetadata> = new Map(); // toolName -> metadata
   ...
   * @returns {Promise<void>}
   */
  public async registerTool(metadata: RegisteredToolMetadata): Promise<void> {
   ...
   */
  public listAllToolMetadata(): RegisteredToolMetadata[] {
   ...
   * @returns {Promise<void>}
   */
  public async updateToolMetrics(toolName: string, executionTime: number, success: boolean): Promise<void> {
   ...
   * @returns {boolean}
   */
  public checkAccess(toolName: string, userId: string, userRoles: string[]): boolean {
    const metadata = this.toolsMetadata.get(toolName);
    if (!metadata) {
   ...
   * @returns {string}
   */
  public getAutoScalingRecommendation(toolName: string): string {
    const metadata = this.toolsMetadata.get(toolName);
    if (!metadata) {
