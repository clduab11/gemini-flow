...
export class AuthenticationManager extends EventEmitter {
  ...
  private tokenRefreshTimers: Map<string, ReturnType<typeof setTimeout>> =
    new Map(); // Auto-refresh timers
  ...
  private async initializeAuth(): Promise<void> {
    try {
      ...
      if (!capabilities.hasGoogleServices) {
        this.logger.warn(
          "Google authentication services not available. Some features may be limited.",
        );
        return;
      }
      ...
      if (
        (this.config.serviceAccountPath || this.config.projectId) &&
  ...
  async authenticateUser(code: string): Promise<UserProfile> {
    ...
    try {
      ...
      const tierResult = await this.detectUserTier(
        userInfo.data.email!,
        tokens,
      );
    ...
    try {
      ...
      try {
        ...
        this.logger.info("Token refresh successful", {
          ...
          hasNewRefreshToken:
            credentials.refresh_token !== storedTokens.refreshToken,
        });
  ...
  private scheduleTokenRefresh(userId: string, tokens: OAuth2Tokens): void {
    ...
    if (!tokens.refreshToken) {
      this.logger.debug(
        "No refresh token available, skipping auto-refresh scheduling",
        { userId },
      );
      return;
    }
  ...
  async detectUserTier(
    ...
    try {
      ...
      // 2. Enterprise Tier Detection (High Priority)
        ...
        const enterpriseResult = await this.detectEnterpriseTier(
          email,
          domain,
          tokens,
        );
  ...
  private async detectUltraTier(
    ...
    try {
      ...
      const hasEnterpriseBilling = await this.checkEnterpriseBilling(
        email,
        domain,
      );
   ...
   */
  private async detectEnterpriseTier(
    email: string,
    domain: string,
    tokens?: any,
  ): Promise<{
    ...
    try {
      ...
      const customPatterns =
        this.config.tierDetection?.customEnterprisePatterns || [];
      ...
      if (projects.data.projects && projects.data.projects.length > 0) {
        for (const project of projects.data.projects) {
          try {
            ...
            const hasVertexAI = services.data.services?.some((service) =>
              service.name?.includes("aiplatform.googleapis.com"),
            );
   ...
   */
  private async checkEnterpriseBilling(
    _email: string,
    _domain: string,
  ): Promise<boolean> {
   ...
   * Check Google Workspace status
   */
  private async checkGoogleWorkspace(
    email: string,
    tokens: any,
  ): Promise<{
    isWorkspace: boolean;
    ...
    try {
      ...
      try {
        ...
        if (domainInfo.data) {
          ...
          const isEnterprise =
            (domainInfo.data as any).verified &&
  ...
  private async checkEnterpriseScopes(tokens: any): Promise<boolean> {
    ...
    const hasEnterpriseScope = enterpriseScopes.some((scope) =>
