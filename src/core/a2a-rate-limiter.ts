...
export class A2ARateLimiter extends EventEmitter {
   ...
   */
  async releaseRequest(agentId: string, success: boolean = true): Promise<void> {
  ...
  private checkSlidingWindows(
    ...
    const recentMinutesCount = agentState.requestCounts.perMinute
      .reduce((sum, count) => sum + count, 0);
    ...
    const recentHoursCount = agentState.requestCounts.perHour
      .reduce((sum, count) => sum + count, 0);
    ...
    // Check per-day limit
    const recentDaysCount = agentState.requestCounts.perDay
      .reduce((sum, count) => sum + count, 0);
  ...
  private applyAdaptiveThrottling(
    ...
    // Check memory usage
    if (this.systemMetrics.memoryUsage > this.config.adaptiveThrottling.memoryThreshold) {
      shouldThrottle = true;
    }
    
    // Check response time
    if (this.systemMetrics.averageResponseTime > this.config.adaptiveThrottling.responseTimeThreshold) {
      shouldThrottle = true;
    }
    ...
    if (shouldThrottle) {
      this.adaptiveMultiplier = Math.max(
        0.1,
        this.adaptiveMultiplier * this.config.adaptiveThrottling.backpressureMultiplier
      );
  ...
  private collectSystemMetrics(): void {
    ...
    this.systemMetrics = {
      ...
      errorRate: this.processingMetrics.requestsBlocked / this.processingMetrics.requestsProcessed,
      timestamp: Date.now()
    };
  ...
  addRule(rule: RateLimitRule): void {
    ...
    this.logger.info('Rate limit rule added', { ruleId: rule.ruleId, name: rule.name });
  }
...
class BehaviorAnalyzer {
  ...
  async analyzeRequest(
    ...
    const totalMessages = Array.from(profile.messageTypeDistribution.values())
      .reduce((sum, count) => sum + count, 0);
