...
export class AuthSystemFactory {
  ...
  static async createAuthSystem(config: AuthSystemConfig) {
    ...
    if (config.enableMCPIntegration) {
      mcpProvider = new MCPAuthenticationProvider(authManager, config.mcpConfig);
    }
    ...
    return {
      ...
      getHealthStatus() {
        return {
          ...
          securityManager: securityManager ? securityManager.getSecurityStats() : null
        };
      }
    };
  }
...
export class AuthHelpers {
   ...
   */
  static validateConfig(config: UnifiedAuthConfig): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    
    // Check if at least one provider is configured
    if (!config.providers.oauth2 && !config.providers.vertexAI && !config.providers.googleAI) {
    ...
    if (config.providers.oauth2) {
      ...
      if (!oauth2.authorizationEndpoint) errors.push('OAuth2 authorizationEndpoint is required');
      if (!oauth2.tokenEndpoint) errors.push('OAuth2 tokenEndpoint is required');
    }
    ...
    if (config.providers.vertexAI) {
      ...
      const hasServiceAccount = !!(vertexAI.serviceAccountKeyPath || vertexAI.serviceAccountKey);
      const hasADC = !!vertexAI.applicationDefaultCredentials;
      
      if (!hasServiceAccount && !hasADC) {
        errors.push('Vertex AI requires either service account key or Application Default Credentials');
      }
    }
