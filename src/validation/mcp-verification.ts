...
import { MCPSettingsManager, MCPSettings } from '../core/mcp-settings-manager.js';
export type { MCPSettings } from '../core/mcp-settings-manager.js';
...
export interface McpVerificationConfig {
  expectedMcpServers: string[]; // List of expected MCP server names
  // Add configuration for expected capabilities, environment variables, etc.
}
 ...
 */
export class McpVerification implements McpVerificationOperations {
  private config: McpVerificationConfig;
   ...
   */
  public async verifySettingsFile(): Promise<boolean> {
   ...
   */
  public async verifyMcpServerTransfer(): Promise<boolean> {
    ...
    if (allTransferred) { this.logger.info('All expected MCP servers transferred successfully.'); }
    else { this.logger.error('Some MCP server transfers failed or are invalid.'); }
   ...
   */
  public async testMcpServerConnectivity(serverName: string): Promise<boolean> {
    ...
    if (isConnectable) { this.logger.info(`MCP server '${serverName}' is connectable.`); }
    else { this.logger.error(`MCP server '${serverName}' is not connectable.`); }
   ...
   */
  public async verifyPermissions(serverName: string): Promise<boolean> {
    ...
    if (permissionsValid) { this.logger.info(`Permissions for '${serverName}' appear valid.`); }
    else { this.logger.error(`Permissions for '${serverName}' are invalid.`); }
   ...
   */
  public async verifyEnvironmentVariables(serverName: string): Promise<boolean> {
    ...
    if (envVarsValid) { this.logger.info(`Environment variables for '${serverName}' appear valid.`); }
    else { this.logger.warn(`Environment variables for '${serverName}' are missing or empty.`); }
    return envVarsValid;
  }
}
