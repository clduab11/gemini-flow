...
export class SyntheticMonitor extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(`Synthetic monitoring started with ${this.config.schedule.interval}min interval`);
    } catch (error) {
  ...
  private async initializeBrowsers(): Promise<void> {
    for (const browserType of this.config.browsers) {
      try {
        ...
        this.logger.error(`Failed to initialize ${browserType} browser:`, error);
  ...
  private async runMonitoringCycle(): Promise<void> {
    ...
    for (const endpoint of this.config.endpoints) {
      for (const location of this.config.locations.filter(l => l.enabled)) {
        for (const [browserName, browser] of this.browsers) {
          try {
            const result = await this.monitorEndpoint(endpoint, location, browserName, browser);
            ...
            this.logger.error(`Error monitoring endpoint ${endpoint.id}:`, error);
          }
        }
      }
    }
    ...
    for (const userFlow of this.config.userFlows) {
      for (const location of this.config.locations.filter(l => l.enabled)) {
        for (const [browserName, browser] of this.browsers) {
          try {
            const result = await this.monitorUserFlow(userFlow, location, browserName, browser);
            cycleResults.push(result);
            ...
            this.logger.error(`Error monitoring user flow ${userFlow.id}:`, error);
          }
        }
      }
    }
    ...
    this.logger.info(`Monitoring cycle completed in ${cycleDuration.toFixed(2)}ms`);
  ...
  private async monitorEndpoint(
    ...
    try {
      ...
      const metrics = await this.collectPerformanceMetrics(page, performanceMetrics);
      ...
      return {
        ...
        responseTime,
        metrics,
        error: success ? undefined : `Status: ${response?.status()}, Expected: ${endpoint.expectedStatus}`
      };
    } catch (error) {
      return {
        id: `${endpoint.id}-${Date.now()}`,
  ...
  private async monitorUserFlow(
    ...
    try {
      ...
      if (responseTime > userFlow.maxDuration) {
        throw new Error(`User flow exceeded maximum duration: ${responseTime}ms > ${userFlow.maxDuration}ms`);
      }
   ...
   */
  private async executeUserFlowStep(page: Page, step: UserFlowStep, stepIndex: number): Promise<void> {
    ...
    switch (step.action) {
      ...
      case 'evaluate':
        if (!step.value) throw new Error('JavaScript code required for evaluate action');
   ...
   * Validate assertion
   */
  private async validateAssertion(page: Page, assertion: UserFlowStep['assertion']): Promise<void> {
    if (!assertion) return;
    ...
    switch (assertion.type) {
      ...
      case 'hidden':
        ...
        if (!isHidden) throw new Error(`Element not hidden: ${assertion.expected}`);
        break;
        
      case 'text':
        const text = await page.textContent(assertion.expected.selector);
        if (text !== assertion.expected.text) {
          throw new Error(`Text mismatch: expected "${assertion.expected.text}", got "${text}"`);
        }
        break;
      ...
      case 'count':
        ...
        if (count !== assertion.expected.count) {
          throw new Error(`Count mismatch: expected ${assertion.expected.count}, got ${count}`);
        }
        break;
      ...
      case 'url':
        ...
        if (!url.includes(assertion.expected)) {
          throw new Error(`URL mismatch: expected to contain "${assertion.expected}", got "${url}"`);
        }
        break;
    }
  }
  ...
  private async startPerformanceMonitoring(page: Page): Promise<any> {
    await page.route('**/*', route => {
      ...
      route.continue().then(() => {
        ...
        this.logger.debug(`Request to ${request.url()} took ${endTime - startTime}ms`);
      });
    });
   ...
   * Collect performance metrics
   */
  private async collectPerformanceMetrics(page: Page, monitoringData: any): Promise<any> {
    try {
   ...
   */
  private async validateEndpointResponse(response: any, endpoint: MonitoringEndpoint): Promise<boolean> {
   ...
   */
  private async generateReports(cycleResults: MonitoringResult[]): Promise<void> {
    const report = {
      timestamp: new Date().toISOString(),
      cycle: {
        duration: cycleResults.length > 0 ? 
          Math.max(...cycleResults.map(r => r.responseTime)) : 0,
      ...
      metrics: {
        averageResponseTime: cycleResults.length > 0 ?
          cycleResults.reduce((sum, r) => sum + r.responseTime, 0) / cycleResults.length : 0,
        successRate: cycleResults.length > 0 ?
          (cycleResults.filter(r => r.success).length / cycleResults.length) * 100 : 0
      },
  ...
  getStatistics(): any {
    ...
    return {
      ...
      averageResponseTime: recentResults.reduce((sum, r) => sum + r.responseTime, 0) / recentResults.length,
      availability: (successfulChecks / recentResults.length) * 100,
      lastCheck: recentResults[recentResults.length - 1]?.timestamp,
      errors: recentResults.filter(r => !r.success).map(r => ({
...
export const DEFAULT_MONITORING_CONFIG: MonitoringConfig = {
  ...
  userFlows: [
    {
      ...
      steps: [
        ...
        { action: 'fill', selector: '[data-testid="password-input"]', value: 'testpassword' },
        { action: 'click', selector: '[data-testid="login-button"]' },
        { action: 'wait', selector: '[data-testid="dashboard"]', assertion: { type: 'visible', expected: '[data-testid="dashboard"]' } }
      ]
    },
    {
      ...
      steps: [
        ...
        { action: 'fill', selector: '[data-testid="video-prompt"]', value: 'Test video generation' },
  ...
  locations: [
    { id: 'us-east-1', name: 'US East (Virginia)', region: 'us-east', enabled: true },
    { id: 'us-west-1', name: 'US West (California)', region: 'us-west', enabled: true },
    { id: 'eu-west-1', name: 'EU West (Ireland)', region: 'eu-west', enabled: true },
    { id: 'ap-southeast-1', name: 'Asia Pacific (Singapore)', region: 'ap-southeast', enabled: true }
  ]
};
