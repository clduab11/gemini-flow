...
export type { EnhancedStreamingConfig, StreamSession, StreamingContext } from './enhanced-streaming-api.js';

// WebRTC architecture
export { WebRTCArchitecture } from './webrtc-architecture.js';
export type { WebRTCPeer, SignalingMessage } from './webrtc-architecture.js';

// Codec management
export { CodecManager } from './codec-manager.js';
export type { CodecCapabilities, TranscodingJob, CodecProfile } from './codec-manager.js';

// Buffer and synchronization
export { BufferSyncManager } from './buffer-sync-manager.js';
export type { BufferMetrics, SyncPoint, BufferPool, ClockReference } from './buffer-sync-manager.js';

// A2A multimedia extension
export { A2AMultimediaExtension } from './a2a-multimedia-extension.js';
export type { A2AStreamingAgent, A2AMultimediaMessage, ConsensusProposal, LoadBalancingStrategy } from './a2a-multimedia-extension.js';

// Quality adaptation
export { QualityAdaptationEngine } from './quality-adaptation-engine.js';
export type { AdaptationContext, DeviceCapabilities, UserPreferences, SessionMetrics, QualityConstraints, AdaptationDecision, MLModel } from './quality-adaptation-engine.js';

// Edge caching and CDN
export { EdgeCacheCDN } from './edge-cache-cdn.js';
export type { CacheEntry, EdgeNode, CacheStrategy, CacheRule, CDNEndpoint, CacheAnalytics } from './edge-cache-cdn.js';
  ...
  return {
    ...
    cdn: {
      ...
      caching: {
        strategy: 'adaptive',
        ttl: 7200000, // 2 hours
        edgeLocations: ['us-east', 'us-west', 'eu-west', 'eu-central', 'ap-southeast', 'ap-northeast']
      },
      optimization: {
        compression: true,
        minification: true,
 ...
 */
export function validateStreamingConfig(config: EnhancedStreamingConfig): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (config.performance.textLatencyTarget > config.performance.multimediaLatencyTarget) {
  ...
  if (config.a2a.enableCoordination) {
    if (config.a2a.consensusThreshold <= 0 || config.a2a.consensusThreshold > 1) {
...
export function createStreamingContext(
  ...
  const defaultContext: StreamingContext = {
    sessionId,
    userPreferences: {
