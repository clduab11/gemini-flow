...
export { ByzantineConsensus, Agent, ConsensusMessage, ConsensusProposal, ConsensusState, default as ByzantineConsensusDefault, } from "./byzantine-consensus";
export { RaftConsensus, RaftNode, LogEntry, RaftMessage, RaftState, default as RaftConsensusDefault, } from "./raft-consensus";
export { VotingMechanisms, Vote, VotingProposal, VotingResult, Voter, VotingType, default as VotingMechanismsDefault, } from "./voting-mechanisms";
export { MaliciousDetection, MaliciousBehavior, ReputationScore, DetectionRule, DetectionContext, SecurityAlert, default as MaliciousDetectionDefault, } from "./malicious-detection";
export { StateMachineReplication, StateOperation, StateSnapshot, ReplicationNode, StateMachineConfig, ConflictResolution, default as StateMachineReplicationDefault, } from "./state-machine-replication";
export { ViewChangeLeaderElection, ViewChangeMessage, NewViewMessage, CheckpointMessage, PreparedSet, LeaderCandidate, ElectionConfiguration, ViewState, default as ViewChangeLeaderElectionDefault, } from "./view-change-leader-election";
export { PerformanceOptimizer, OptimizationConfig, PerformanceMetrics, BatchedProposal, PipelineStage, SpeculativeExecution, AdaptiveThreshold, OptimizedResult, default as PerformanceOptimizerDefault, } from "./performance-optimizer";
// Security integration
export { ConsensusSecurityIntegration, SecureConsensusConfig, SecureConsensusSession, SecuredConsensusProposal, default as ConsensusSecurityIntegrationDefault, } from "./consensus-security-integration";
...
export const QUORUM_CALCULATIONS = {
     ...
     */
    calculateByzantineQuorum: (totalAgents) => {
     ...
     */
    calculateRaftQuorum: (totalAgents) => {
     ...
     */
    calculateGossipQuorum: (totalAgents, threshold = 0.51) => {
...
export const BYZANTINE_FAULT_TOLERANCE = {
     ...
     */
    calculateFaultThreshold: (totalAgents) => {
     ...
     */
    calculateMinimumAgents: (faultyAgents) => {
     ...
     */
    canReachConsensus: (totalAgents, faultyAgents) => {
...
export const MALICIOUS_BEHAVIOR_TYPES = {
    ...
    CONFLICTING_MESSAGES: "conflicting-messages",
    TIMING_MANIPULATION: "timing-manipulation",
    FAKE_SIGNATURES: "fake-signatures",
    SPAM_FLOODING: "spam-flooding",
    COLLUSION: "collusion",
    VIEW_CHANGE_ABUSE: "view-change-abuse",
    CONSENSUS_DISRUPTION: "consensus-disruption",
    SYBIL_ATTACK: "sybil-attack",
    ECLIPSE_ATTACK: "eclipse-attack",
};
export const VOTING_TYPES = {
    ...
    STAKE_WEIGHTED: "stake-weighted",
};
/**
 * Factory function to create a Raft consensus system
 */
export function createRaftConsensusSystem(nodeId, totalNodes = 3) {
 ...
 */
export function createByzantineConsensusSystem(nodeId, totalAgents = 7, securityManager, options = {}) {
    const { enableSecurity = true, enableOptimization = true, enableMaliciousDetection = true, securityLevel = "medium", } = options;
    ...
    if (enableSecurity && securityManager) {
        securityIntegration = new ConsensusSecurityIntegration(nodeId, securityManager, {
            ...
            securityPolicies: {
                minTrustLevel: securityLevel === "critical"
                    ? "trusted"
                    : securityLevel === "high"
                        ? "verified"
                        : "basic",
 ...
 */
export function validateConsensusConfig(totalAgents, expectedFaultyAgents) {
    const faultThreshold = BYZANTINE_FAULT_TOLERANCE.calculateFaultThreshold(totalAgents);
...
export const PERFORMANCE_BENCHMARKS = {
     ...
     */
    measureConsensusLatency: async (consensus, proposal) => {
     ...
     */
    measureVotingThroughput: async (voting, votes) => {
        const startTime = Date.now();
        let successful = 0;
