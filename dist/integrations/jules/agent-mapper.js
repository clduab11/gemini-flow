...
export class JulesAgentMapper {
    ...
    async selectAgents(context) {
        ...
        const primary = this.selectPrimaryAgents(mapping.agents, complexity, context.priority);
        // Select validator agents for consensus
        const validators = this.selectValidatorAgents(mapping.validators, context.requiresConsensus !== false // Default to true
        );
        // Select coordination agents
        const coordinators = this.selectCoordinators(mapping.coordinators, complexity);
        const selection = {
            primary,
            validators,
            coordinators,
    ...
    getAgentsByCategory(category) {
        return Object.values(this.agentDefinitions).filter(agent => agent.category === category);
    }
    /**
     ...
     */
    selectPrimaryAgents(agentIds, complexity, priority) {
        const countMap = {
            'simple': 1,
            'moderate': 2,
            'complex': 3,
     ...
     * Select validator agents for consensus
     */
    selectValidatorAgents(categories, requiresConsensus) {
     ...
     * Select coordination agents
     */
    selectCoordinators(categories, complexity) {
        const coordinators = [];
     ...
     */
    calculateConfidence(primary, validators, coordinators) {
     ...
     */
    generateReasoningExplanation(context, primary, validators, coordinators) {
        const parts = [];
     ...
     */
    getTaskMapping(taskType) {
        return this.TASK_AGENT_MAPPING[taskType];
    }
    /**
