...
import { JulesError, JulesErrorType, } from './types.js';
...
export class JulesCliWrapper {
    ...
    async install() {
        try {
            ...
            const { stdout, stderr } = await execAsync('npm install -g @google/jules-tools', { timeout: 300000 } // 5 minutes for installation
            );
    ...
    async initialize(config) {
        try {
            ...
            if (!this.config.apiKey && !process.env.JULES_API_KEY) {
                throw new JulesError('Jules API key is required. Set JULES_API_KEY environment variable or provide apiKey in config.', JulesErrorType.INVALID_CONFIGURATION);
            }
    ...
    async createTask(params) {
        ...
        try {
            ...
            const response = await this.apiRequest('POST', '/tasks', taskData);
            if (!response.success || !response.data) {
                throw new JulesError(response.error?.message || 'Failed to create task', JulesErrorType.TASK_CREATION_FAILED, { details: response.error });
            }
            ...
            throw new JulesError(`Failed to create Jules task: ${error.message}`, JulesErrorType.TASK_CREATION_FAILED, { cause: error });
        }
    }
    /**
    ...
    async getTask(taskId) {
        this.ensureInitialized();
        try {
            const response = await this.apiRequest('GET', `/tasks/${taskId}`);
            if (!response.success || !response.data) {
                throw new JulesError(response.error?.message || 'Task not found', JulesErrorType.TASK_NOT_FOUND, { taskId, details: response.error });
            }
            return response.data;
        }
        catch (error) {
            ...
            throw new JulesError(`Failed to get task: ${error.message}`, JulesErrorType.TASK_NOT_FOUND, { taskId, cause: error });
        }
    }
    /**
    ...
    async cancelTask(taskId) {
        ...
        try {
            ...
            const response = await this.apiRequest('DELETE', `/tasks/${taskId}`);
            if (!response.success) {
                throw new JulesError(response.error?.message || 'Failed to cancel task', JulesErrorType.API_ERROR, { taskId, details: response.error });
            }
            ...
            throw new JulesError(`Failed to cancel task: ${error.message}`, JulesErrorType.API_ERROR, { taskId, cause: error });
        }
    }
    /**
    ...
    async listTasks(filter) {
        ...
        try {
            ...
            if (!response.success || !response.data) {
                throw new JulesError(response.error?.message || 'Failed to list tasks', JulesErrorType.API_ERROR, { details: response.error });
            }
            return response.data;
        }
        catch (error) {
            ...
            throw new JulesError(`Failed to list tasks: ${error.message}`, JulesErrorType.API_ERROR, { cause: error });
        }
    }
    /**
     * Stream task logs in real-time
     */
    async streamTaskLogs(taskId, callback) {
        this.ensureInitialized();
        try {
    ...
    async validateGitHubConnection() {
        try {
            ...
            // Validate via API
            const response = await this.apiRequest('GET', '/github/validate', undefined, { 'X-GitHub-Token': githubToken });
            if (!response.success || !response.data) {
                return {
                    connected: false,
    ...
    async apiRequest(method, endpoint, data, headers) {
        ...
        throw new JulesError(`API request failed after ${this.config.retryAttempts} attempts: ${lastError?.message}`, JulesErrorType.API_ERROR, { cause: lastError });
    }
    /**
     * Stream logs via CLI
     */
    async streamLogsViaCLI(taskId, callback) {
        return new Promise((resolve, reject) => {
            ...
            proc.on('close', (code) => {
                ...
                if (code === 0) {
                    ...
                    reject(new JulesError(`Jules CLI exited with code ${code}`, JulesErrorType.CLI_ERROR, { taskId }));
                }
            });
            proc.on('error', (error) => {
                this.logCallbacks.delete(taskId);
                reject(new JulesError(`Jules CLI error: ${error.message}`, JulesErrorType.CLI_ERROR, { taskId, cause: error }));
            });
        });
    }
    /**
     ...
     */
    async pollTaskLogs(taskId, callback) {
        let lastLogIndex = 0;
    ...
    ensureInitialized() {
        if (!this.initialized) {
            throw new JulesError('Jules wrapper not initialized. Call initialize() first.', JulesErrorType.INVALID_CONFIGURATION);
        }
    }
    /**
     * Sleep utility
