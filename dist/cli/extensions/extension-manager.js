...
export class ExtensionManager extends EventEmitter {
    ...
    async installExtension(source, options = {}) {
        ...
        if (match) {
            ...
            try {
                ...
                await fs.writeFile(path.join(extensionPath, "extension.json"), JSON.stringify(manifest, null, 2));
            ...
            throw new Error("Invalid source format. Use 'github:user/repo' or full GitHub URL");
        }
    }
    ...
    async executeCommand(extensionName, commandName, args) {
        const extension = this.extensions.get(extensionName);
        if (!extension)
            throw new Error(`Extension '${extensionName}' not found`);
        if (!extension.enabled)
            throw new Error(`Extension '${extensionName}' is disabled`);
