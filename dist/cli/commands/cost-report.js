...
export class CostReportCommand extends Command {
    ...
    async costReportAction(options) {
        ...
        try {
            ...
            const finalReport = this.compileFinalReport(costData, usageAnalysis, costBreakdown, optimizationReport, forecast, roiAnalysis, options);
    ...
    async analyzeUsagePatterns(costData, options) {
        const analysis = {
            ...
            userBehavior: {
                averageSessionCost: costData.totalCost /
    ...
    async generateCostBreakdown(costData, options) {
        const breakdown = {
            summary: {
            ...
            byCategory: {
                "Model Usage": Object.values(costData.modelCosts).reduce((sum, model) => sum + (Number(model?.cost) || 0), 0),
        ...
        // Model breakdown
        Object.entries(costData.modelCosts).forEach(([model, data]) => {
        ...
        if (options.breakdownByTier) {
            Object.entries(costData.tierCosts).forEach(([tier, data]) => {
    ...
    async generateOptimizationReport(costData, usageAnalysis, options) {
        const totalPotentialSavings = usageAnalysis.wasteIdentification.reduce((sum, waste) => sum + waste.potentialSavings, 0);
        const optimization = {
            summary: {
                currentCost: costData.totalCost,
                potentialSavings: totalPotentialSavings,
                optimizedCost: costData.totalCost - totalPotentialSavings,
                savingsPercentage: ((totalPotentialSavings / costData.totalCost) *
                    100).toFixed(1),
            },
            recommendations: [
                {
    ...
    async generateCostForecast(costData, options) {
    ...
    async generateROIAnalysis(costData, usageAnalysis, options) {
        ...
        roi.calculation.totalBenefits = Object.values(roi.costAvoidance).reduce((sum, value) => sum + value, 0);
    ...
    compileFinalReport(costData, usageAnalysis, costBreakdown, optimizationReport, forecast, roiAnalysis, options) {
        const report = {
            metadata: {
            ...
            executiveSummary: {
                ...
                topRecommendation: optimizationReport?.recommendations[0]?.description ||
    ...
    displayCostReport(report, options) {
        ...
        Object.entries(report.costBreakdown.byModel).forEach(([model, data]) => {
        ...
        if (report.costBreakdown.byTier &&
            ...
            Object.entries(report.costBreakdown.byTier).forEach(([tier, data]) => {
    ...
    displayCostAlerts(alerts, options) {
        if (alerts.length === 0) {
