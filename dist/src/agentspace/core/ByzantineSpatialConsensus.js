...
import { ByzantineConsensus, } from "../../protocols/a2a/consensus/byzantine-consensus.js";
export class ByzantineSpatialConsensus extends EventEmitter {
    ...
    constructor(agentId, config, totalAgents = 7) {
        super();
     ...
     */
    async proposeLocationChange(agentId, targetLocation, movementPath = [], priority = "normal") {
     ...
     */
    async proposeResourceAllocation(agentId, resourceType, amount, duration, exclusiveAccess = false) {
        const agent = this.spatialAgents.get(agentId);
        if (!agent) {
            throw new Error(`Agent not found: ${agentId}`);
        }
     ...
     * Propose zone access
     */
    async proposeZoneAccess(agentId, zoneId, accessType, duration, modifications) {
     ...
     */
    async proposeCollaboration(initiatorId, participantIds, collaborationType, spatialArrangement, duration) {
        const proposal = {
     ...
     */
    async voteOnProposal(proposalId, voterId, vote, reasoning, conditions) {
     ...
     */
    async resolveConflicts(conflictIds) {
        const resolutions = [];
    ...
    async checkConsensusCompletion(proposalId) {
        const proposal = this.activeProposals.get(proposalId);
    ...
    calculateConsensusResult(proposal, votes) {
    ...
    async finalizeConsensus(proposal, result) {
    ...
    async implementConsensusResult(result) {
    ...
    async executeImplementationStep(step) {
    ...
    async detectLocationConflicts(agentId, targetLocation) {
        const conflicts = [];
        const agent = this.spatialAgents.get(agentId);
        if (!agent)
            return conflicts;
    ...
    getAffectedAgents(location, boundingBox) {
        const affected = [];
    ...
    calculateMovementTime(from, to) {
        const distance = this.calculateDistance(from, to);
        const speed = 5; // units per second
        return (distance / speed) * 1000; // milliseconds
    }
    analyzeResourceImpact(agent, targetLocation) {
    ...
    getResourceContenders(resourceType, amount) {
        const contenders = [];
        for (const [agentId, agent] of this.spatialAgents) {
    ...
    determineResourcePriority(resourceType, amount) {
    ...
    generateResourceFallbacks(resourceType, amount) {
        return [
            {
                resourceType,
    ...
    determineZoneAccessPriority(accessType) {
        switch (accessType) {
            case "exit":
    ...
    calculateCollaborationResources(participantIds, collaborationType) {
    ...
    checkAgentConflict(agent1, agent2) {
    ...
    checkResourceConflicts() {
        const conflicts = [];
        const resourceMap = new Map(); // Resource -> Agent IDs
    ...
    resolveLocationOverlap(conflict) {
    ...
    async implementResolution(resolution) {
    ...
    isLocationInZone(location, zone) {
        ...
        return (location.x >= bounds.min.x &&
    ...
    findNearbyFreeLocation(currentLocation) {
    ...
    signVote(proposalId, voterId, vote) {
    ...
    handleBaseConsensusResult(proposal, success) {
        this.logger.debug("Base consensus result received", {
            proposalId: proposal.id,
            success,
        });
    }
    buildFinalConfiguration(proposal, conditionalVotes) {
    ...
    buildImplementationPlan(proposal, decision) {
...
//# sourceMappingURL=ByzantineSpatialConsensus.js.map