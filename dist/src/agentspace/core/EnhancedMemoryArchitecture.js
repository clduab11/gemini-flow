...
export class EnhancedMemoryArchitecture extends EventEmitter {
    ...
    config;
    baseMemoryManager;
    spatialMemoryNodes = new Map();
    spatialIndex = new Map(); // Grid -> Node IDs
    knowledgeGraph = new Map();
    proximityIndex = new Map();
    temporalIndex = new Map(); // Time bucket -> Nodes
    analyticsCache = {
        analytics: null,
    ...
    constructor(config, baseMemoryManager) {
        super();
     ...
     */
    async queryMemoryBySpatialProximity(location, radius, memoryTypes, limit = 50) {
        ...
        try {
            ...
            results.sort((a, b) => this.calculateDistance(location, a.location) -
     ...
     */
    async queryMemoryByKnowledgeGraph(startNodeId, traversalDepth = 2, linkTypes, limit = 100) {
        if (!this.config.knowledgeGraphEnabled) {
        ...
        const paths = [];
        try {
            await this.traverseKnowledgeGraph(startNodeId, [], traversalDepth, linkTypes, visited, results, paths, limit);
            this.updateQueryMetrics(startTime);
    ...
    async queryMemory(query) {
        ...
        try {
            ...
            switch (query.type) {
                case "spatial":
                    candidates = await this.queryMemoryBySpatialProximity(query.parameters.location, query.parameters.radius, query.parameters.memoryTypes, query.limit);
                    break;
                case "temporal":
                    candidates = this.queryByTemporalRange(query.parameters.timeRange.start, query.parameters.timeRange.end);
                    break;
                case "semantic":
                    candidates = await this.queryBySemantic(query.parameters.semantic, query.parameters.agentId);
                    break;
                case "knowledge_graph":
                    const graphResult = await this.queryMemoryByKnowledgeGraph(query.parameters.agentId, // Using as start node
                    3, undefined, query.limit);
                    candidates = graphResult.nodes;
                    break;
                case "full_text":
                    candidates = await this.queryByFullText(query.parameters.keywords, query.parameters.memoryTypes);
                    break;
                default:
     ...
     */
    async updateMemoryNode(nodeId, updates) {
        const node = this.spatialMemoryNodes.get(nodeId);
        if (!node) {
     ...
     */
    async synchronizeWithNearbyAgents(agentLocation, syncRadius) {
        const nearbyNodes = await this.queryMemoryBySpatialProximity(agentLocation, syncRadius);
     ...
     */
    async getMemoryAnalytics(forceRefresh = false) {
    ...
    getSpatialCandidates(location, radius) {
    ...
    removeKnowledgeGraphNode(nodeId) {
    ...
    async updateProximityIndex(node) {
        const neighbors = await this.queryMemoryBySpatialProximity(node.location, this.config.spatialRadius, undefined, 20);
        const proximityNeighbors = neighbors
    ...
    async traverseKnowledgeGraph(currentNodeId, currentPath, remainingDepth, linkTypes, visited, results, paths, limit) {
        if (remainingDepth === 0 ||
        ...
        for (const link of links) {
            if (linkTypes && !linkTypes.includes(link.linkType))
                continue;
            await this.traverseKnowledgeGraph(link.targetNodeId, [...currentPath, currentNodeId], remainingDepth - 1, linkTypes, visited, results, paths, limit);
        }
    }
    queryByTemporalRange(start, end) {
        const results = [];
    ...
    async queryBySemantic(semantic, agentId) {
    ...
    async queryByFullText(keywords, memoryTypes) {
        const results = [];
        for (const node of this.spatialMemoryNodes.values()) {
            if (memoryTypes && !memoryTypes.includes(node.memoryType))
                continue;
            const dataString = JSON.stringify(node.data).toLowerCase();
            const matches = keywords.some((keyword) => dataString.includes(keyword.toLowerCase()));
            if (matches) {
                results.push(node);
            }
        }
    ...
    evaluateFilterCondition(value, operator, filterValue) {
        switch (operator) {
            case "equals":
    ...
    async propagateToDistributedMemory(operation) {
    ...
    handleDistributedMemoryDelta(delta) {
    ...
    analyzeSpatialDistribution(locations) {
    ...
    determineRelationshipType(node1, node2) {
        if (node1.memoryType === node2.memoryType)
            return "similar_type";
        if (node1.agentId === node2.agentId)
            return "same_agent";
        return "proximity";
    }
    calculateRelationshipWeight(node1, node2) {
    ...
    calculateInteractionStrength(neighbors) {
...
//# sourceMappingURL=EnhancedMemoryArchitecture.js.map