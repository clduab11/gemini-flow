...
export class LearnCommand extends Command {
    ...
    async analyzeCodebase(sourcePath, options) {
        const data = {
            path: sourcePath,
            fileCount: 0,
            languages: {},
        ...
        try {
            ...
            if (options.days && existsSync(join(sourcePath, ".git"))) {
                try {
                    const gitLog = execSync(`cd "${sourcePath}" && git log --since="${options.days} days ago" --name-only --pretty=format:"%h|%an|%ad|%s" --date=short | head -200`, { encoding: "utf8" });
                    if (gitLog.trim()) {
