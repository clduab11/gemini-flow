...
export class CodecManager extends EventEmitter {
     ...
     */
    getOptimalCodec(type, constraints) {
     ...
     */
    async startTranscoding(sourceData, sourceCodec, targetCodec, quality) {
     ...
     */
    isHardwareAccelerated(codecName) {
     ...
     */
    getSupportedCodecs(type) {
        return Array.from(this.supportedCodecs.values()).filter((codec) => this.isCodecType(codec, type));
    }
    /**
     * Create adaptive bitrate ladder
     */
    createAdaptiveBitrateConfigs(baseCodec, maxBitrate) {
        const configs = [];
    ...
    async processTranscodingJob(job) {
        try {
            ...
            job.status = "failed";
            this.emit("transcoding_failed", { job, error: error.message });
        }
    }
    /**
    ...
    checkHardwareSupport(codec) {
        ...
        return hardwareSupported[codec] || false;
    }
    /**
     ...
     */
    meetsConstraints(codec, constraints) {
        if (constraints.hardwareAcceleration && !codec.hardwareAccelerated) {
            return false;
        }
        if (constraints.compatibility) {
            const hasCompatibleFormat = constraints.compatibility.some((format) => codec.supportedFormats.includes(format));
    ...
    scoreCodec(codec, constraints) {
        ...
        if (constraints.bandwidth && constraints.bandwidth < 1000000) {
            ...
            if (codec.name === "VP9" || codec.name === "AV1")
                score += 4;
            if (codec.name === "Opus")
                score += 4;
        }
     ...
     */
    createMediaCodec(profile, quality) {
     ...
     */
    async detectFormatFromBinary(data) {
...
//# sourceMappingURL=codec-manager.js.map