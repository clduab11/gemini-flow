...
export class SLAComplianceMonitor extends EventEmitter {
    ...
    constructor(config, metricsCollector, syntheticMonitor) {
        super();
        this.config = config;
     ...
     */
    calculateAvailability(windowStart, windowEnd) {
     ...
     * Calculate performance metrics
     */
    calculatePerformanceMetrics(windowStart, windowEnd) {
     ...
     * Calculate reliability metrics
     */
    calculateReliabilityMetrics(windowStart, windowEnd) {
     ...
     * Calculate throughput metrics
     */
    calculateThroughputMetrics(windowStart, windowEnd) {
     ...
     */
    checkCompliance(availability, performance, reliability, throughput) {
     ...
     */
    async checkForViolations(measurement) {
        const violations = [];
     ...
     */
    determineImpact(actual, target, higher_is_worse = false) {
     ...
     */
    calculateCredit(violation) {
        if (violation.type !== "availability")
            return null;
        // Find applicable penalty threshold
        const threshold = this.config.penalties.thresholds.find((t) => violation.actual < t.availabilityBelow);
        if (!threshold)
            return null;
        return {
     ...
     * Handle escalation
     */
    async handleEscalation(violation) {
        // Find appropriate escalation level
        const violationDuration = this.calculateContinuousViolationDuration(violation.type);
        const escalationLevel = this.config.escalation.levels.find((level) => violationDuration >= level.triggerAfter);
        if (!escalationLevel ||
     ...
     */
    calculateContinuousViolationDuration(violationType) {
     ...
     */
    async executeEscalationAction(action, violation) {
        try {
            switch (action.type) {
                case "alert":
    ...
    recordHealthCheckMetrics(result) {
        this.metricsCollector.recordMetric("health_check_response_time", result.responseTime, "histogram", { endpoint: result.endpoint, status: result.status }, "ms");
        this.metricsCollector.recordMetric("health_check_status", result.status === "up" ? 1 : 0, "gauge", { endpoint: result.endpoint });
    }
    /**
     * Record SLA metrics
     */
    recordSLAMetrics(measurement) {
        ...
        this.metricsCollector.recordMetric("sla_compliance_overall", measurement.compliance.overall ? 1 : 0, "gauge");
        this.metricsCollector.recordMetric("sla_compliance_availability", measurement.compliance.availability ? 1 : 0, "gauge");
        this.metricsCollector.recordMetric("sla_compliance_response_time", measurement.compliance.responseTime ? 1 : 0, "gauge");
        this.metricsCollector.recordMetric("sla_compliance_error_rate", measurement.compliance.errorRate ? 1 : 0, "gauge");
    }
    /**
     * Start reporting
     */
    ...
    async generateReport(type) {
        ...
        const periodMeasurements = this.measurements.filter((m) => m.timestamp >= start.getTime() && m.timestamp <= now.getTime());
        const periodViolations = this.violations.filter((v) => v.timestamp >= start.getTime() && v.timestamp <= now.getTime());
        const periodCredits = this.credits.filter((c) => c.timestamp >= start.getTime() && c.timestamp <= now.getTime());
        // Calculate summary
     ...
     * Generate recommendations
     */
    generateRecommendations(measurements, violations) {
     ...
     * Setup listeners
     */
    setupSyntheticMonitoringListeners() {
     ...
     * Record incident
     */
    recordIncident(id, startTime, endTime) {
    ...
    getCurrentSLAStatus() {
        ...
        const recentViolations = this.violations.filter((v) => Date.now() - v.timestamp < 24 * 60 * 60 * 1000);
        return {
...
//# sourceMappingURL=sla-compliance-monitor.js.map