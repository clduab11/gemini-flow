...
export class SyntheticMonitor extends EventEmitter {
     ...
     */
    async monitorEndpoint(endpoint, location, browserName, browser) {
     ...
     * Monitor a user flow
     */
    async monitorUserFlow(userFlow, location, browserName, browser) {
        const startTime = performance.now();
        const context = await browser.newContext({
            userAgent: `GeminiFlow-SyntheticMonitor/1.0 (${browserName}; ${location.region})`,
            recordVideo: {
        ...
        try {
            ...
            for (let i = 0; i < userFlow.steps.length; i++) {
                ...
                try {
                    ...
                    const screenshotPath = path.join(process.cwd(), "monitoring", "screenshots", `${userFlow.id}-${browserName}-${location.id}-step${i}.png`);
            ...
            // Stop tracing
            const tracePath = path.join(process.cwd(), "monitoring", "traces", `${userFlow.id}-${browserName}-${location.id}.zip`);
     ...
     */
    async executeUserFlowStep(page, step, stepIndex) {
     ...
     * Validate assertion
     */
    async validateAssertion(page, assertion) {
        if (!assertion)
            return;
     ...
     * Collect performance metrics
     */
    async collectPerformanceMetrics(page, monitoringData) {
        try {
     ...
     */
    async validateEndpointResponse(response, endpoint) {
        if (!response)
            return false;
        // Check status code
     ...
     */
    async handleCriticalFailure(result) {
     ...
     */
    async generateReports(cycleResults) {
        const report = {
        ...
        // Save report
        const reportPath = path.join(process.cwd(), "monitoring", "reports", `synthetic-monitoring-${Date.now()}.json`);
    ...
    getStatistics() {
        ...
        return {
            ...
            averageResponseTime: recentResults.reduce((sum, r) => sum + r.responseTime, 0) /
...
//# sourceMappingURL=synthetic-monitoring.js.map