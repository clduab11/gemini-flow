...
import { getTracing, DEFAULT_TRACING_CONFIG, } from "./distributed-tracing";
...
export class ProductionMonitoringSystem extends EventEmitter {
     ...
     */
    async initializeCustomMetrics() {
        try {
            this.metricsCollector = new CustomMetricsCollector(DEFAULT_METRICS_CONFIG, this.distributedTracing);
     ...
     * Initialize synthetic monitoring
     */
    async initializeSyntheticMonitoring() {
        try {
     ...
     */
    async initializeRealUserMonitoring() {
        try {
    ...
    async initializeSLACompliance() {
        try {
            ...
            this.slaMonitor = new SLAComplianceMonitor(DEFAULT_SLA_CONFIG, this.metricsCollector, this.syntheticMonitor);
     ...
     */
    async startSyntheticMonitoring() {
        if (!this.syntheticMonitor)
            return;
        try {
     ...
     */
    async handleCriticalAlert(source, alert) {
     ...
     */
    async handleSLAEscalation(escalation) {
    ...
    async sendAlert(alert) {
        const applicableChannels = this.config.alerting.channels.filter((channel) => channel.severity.includes(alert.severity));
     ...
     */
    updateComponentHealth(component, status, error) {
...
export const DEFAULT_MONITORING_SYSTEM_CONFIG = {
...
// Singleton instance
let monitoringSystemInstance = null;
/**
 * Get or create the monitoring system instance
 */
export function getMonitoringSystem(config) {
    if (!monitoringSystemInstance) {
        monitoringSystemInstance = new ProductionMonitoringSystem(config || DEFAULT_MONITORING_SYSTEM_CONFIG);
    }
...
//# sourceMappingURL=production-monitoring-system.js.map