...
export class AgentMemory {
    ...
    memoryIntelligence;
    agentId;
    constructor(agentId, dbCore, memoryIntelligence) {
     ...
     */
    async storeMemory(key, value, namespace = 'agent_private') {
     ...
     */
    async retrieveMemory(key, namespace = 'agent_private') {
     ...
     * @returns {Promise<void>}
     */
    async storeContext(sessionId, contextData, expiresAt) {
        ...
        if (existingContext) {
            ...
            await this.dbCore.runQuery(`INSERT INTO contexts (id, agent_id, session_id, data, last_accessed, expires_at)
         VALUES (?, ?, ?, ?, ?, ?)`, newContext.id, newContext.agent_id, newContext.session_id, newContext.data, newContext.last_accessed, newContext.expires_at);
        }
    ...
    async retrieveContext(sessionId) {
        const id = `${this.agentId}-${sessionId}`;
        const context = await this.dbCore.getQuery('SELECT * FROM contexts WHERE id = ? AND expires_at > ?', [id, Date.now()]);
        if (context) {
     ...
     * @returns {Promise<any[]>}
     */
    async searchContextAware(currentQuery, tables, limit = 5) {
     ...
     * @returns {Promise<void>}
     */
    async recordPerformanceMetric(metricName, metricValue, entityType = 'agent', entityId = this.agentId, metadata) {
        ...
        await this.dbCore.runQuery(`INSERT INTO performance (id, entity_id, entity_type, metric_name, metric_value, timestamp, metadata)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, newMetric.id, newMetric.entity_id, newMetric.entity_type, newMetric.metric_name, newMetric.metric_value, newMetric.timestamp, newMetric.metadata);
     ...
     */
    async getAgentPerformanceMetrics(metricName, limit = 10) {
        return this.dbCore.allQuery('SELECT * FROM performance WHERE entity_id = ? AND entity_type = ? AND metric_name = ? ORDER BY timestamp DESC LIMIT ?', [this.agentId, 'agent', metricName, limit]);
    }
    // Placeholder for memory-based learning and adaptation mechanisms
    async adaptBasedOnMemory() {
