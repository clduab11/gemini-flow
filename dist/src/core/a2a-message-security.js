...
export class A2AMessageSecurity extends EventEmitter {
     ...
     */
    async secureMessage(message, fromAgentId, toAgentId, options = {}) {
     ...
     */
    async verifyMessage(secureMessage, receivingAgentId) {
        const startTime = Date.now();
        const errors = [];
        try {
            ...
            if (this.config.replayProtection.enabled) {
                const isReplay = await this.checkReplayAttack(secureMessage.security.nonce, secureMessage.header.from, secureMessage.header.timestamp);
                if (isReplay) {
            ...
            if (secureMessage.payload.encrypted) {
                try {
                    const decryptedData = await this.decryptPayload(secureMessage.payload.data, secureMessage.header.from, receivingAgentId, secureMessage.security.keyId);
                    // Decompress if needed
                    let finalData = decryptedData;
                    if (secureMessage.security.compression &&
            ...
            const calculatedChecksum = crypto
                .createHash("sha256")
                .update(secureMessage.payload.encrypted
            ...
            if (this.config.replayProtection.enabled) {
                await this.storeNonce(secureMessage.security.nonce, secureMessage.header.from, secureMessage.messageId, secureMessage.header.timestamp);
            }
            this.metrics.messagesProcessed++;
            // Update performance metrics
            const processingTime = Date.now() - startTime;
     ...
     */
    async encryptPayload(data, fromAgentId, toAgentId, keyId) {
     ...
     */
    async decryptPayload(encryptedData, fromAgentId, toAgentId, keyId) {
        const algorithm = this.config.defaultEncryption;
     ...
     */
    async signMessage(message, agentId, keyId) {
        const startTime = Date.now();
     ...
     */
    async generateHMAC(message, agentId) {
    ...
    async verifyHMAC(message) {
        try {
            ...
            return crypto.timingSafeEqual(Buffer.from(message.security.mac, "hex"), Buffer.from(calculatedMAC, "hex"));
        }
        catch (error) {
     ...
     * Check for replay attacks
     */
    async checkReplayAttack(nonce, agentId, timestamp) {
     ...
     * Store nonce for replay protection
     */
    async storeNonce(nonce, agentId, messageId, timestamp) {
        const record = {
            nonce,
            agentId,
            timestamp,
        ...
        await this.cache.set(`a2a:nonce:${nonce}`, record, this.config.replayProtection.maxNonceAge * 1000);
     ...
     */
    async getOrCreateEphemeralKey(agentId) {
     ...
     */
    async compressData(data, algorithm) {
     ...
     */
    async getEncryptionKey(fromAgentId, toAgentId, _keyId) {
    ...
    async getPublicKey(agentId, keyId) {
    ...
    updatePerformanceMetrics(operation, time) {
        ...
        this.metrics.performanceStats[`avg${operation.charAt(0).toUpperCase() + operation.slice(1)}Time`] = (current + time) / 2;
    }
...
//# sourceMappingURL=a2a-message-security.js.map