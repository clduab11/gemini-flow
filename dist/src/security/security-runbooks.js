...
export class SecurityRunbooks extends EventEmitter {
     ...
     */
    async executeAllSteps(executionId) {
        const execution = this.executions.get(executionId);
        const runbook = this.runbooks.get(execution.runbookId);
    ...
    async executeStep(execution, step) {
        ...
        try {
            ...
            if (step.prerequisites?.length) {
                const prerequisitesMet = await this.checkPrerequisites(step.prerequisites, execution);
                if (!prerequisitesMet) {
    ...
    async executeCommand(action, execution) {
        if (!action.command) {
    ...
    async executeApiCall(action, execution) {
        ...
        const parameters = this.substituteVariables(JSON.stringify(action.parameters || {}), execution.variables);
    ...
    async sendNotification(action, execution) {
        const message = this.substituteVariables(action.parameters?.message || action.description, execution.variables);
    ...
    async createDocumentation(action, execution) {
        const content = this.substituteVariables(action.parameters?.template || action.description, execution.variables);
    ...
    async performAnalysis(action, execution) {
    ...
    async checkPrerequisites(prerequisites, execution) {
    ...
    addCertificateExpirationRunbook() {
    ...
    addVulnerabilityDisclosureRunbook() {
...
//# sourceMappingURL=security-runbooks.js.map