...
export class DynamicAdapterLoader extends EventEmitter {
     ...
     */
    async loadAdapter(key, forceReload = false) {
     ...
     */
    async performLoad(spec) {
        const startTime = performance.now();
        this.logger.info(`Loading adapter: ${spec.name}...`);
        // Check if feature is enabled
        if (!featureFlags.isEnabled(spec.featureFlag)) {
        ...
        // Check dependencies
        const dependencyCheck = await this.checkDependencies(spec);
     ...
     */
    async loadFallback(originalSpec) {
    ...
    async loadEnabledAdapters() {
        ...
        for (const [key, spec] of this.adapterSpecs) {
            ...
            if (featureFlags.isEnabled(spec.featureFlag) || spec.required) {
                try {
    ...
    async isAvailable(key) {
        ...
        if (!featureFlags.isEnabled(spec.featureFlag))
            return false;
        return (await this.checkDependencies(spec)).available;
    }
    /**
     * Get adapter status
     */
    async getAdapterStatus(key) {
        const spec = this.adapterSpecs.get(key);
        const loaded = this.loadedAdapters.get(key);
        if (!spec) {
            return { status: "unknown", key };
        }
        const deps = await this.checkDependencies(spec);
        return {
            key,
            name: spec.name,
            required: spec.required,
            experimental: spec.experimental,
            featureEnabled: featureFlags.isEnabled(spec.featureFlag),
    ...
    async getAllStatuses() {
        const statuses = await Promise.all(Array.from(this.adapterSpecs.keys()).map((key) => this.getAdapterStatus(key)));
        return statuses;
    }
    /**
     * Load adapter on demand
     */
    async loadOnDemand(key) {
        try {
            const adapter = await this.loadAdapter(key);
    ...
    async shutdown() {
        ...
        const shutdownPromises = Array.from(this.loadedAdapters.keys()).map((key) => this.unloadAdapter(key));
...
export function getAdapterLoader() {
    if (!loaderInstance) {
        loaderInstance = new DynamicAdapterLoader();
    }
    return loaderInstance;
}
export function resetAdapterLoader() {
    if (loaderInstance) {
        loaderInstance.shutdown();
        loaderInstance = null;
    }
}
//# sourceMappingURL=dynamic-adapter-loader.js.map