...
export class JulesWorkflowAdapter extends BaseModelAdapter {
    ...
    async generate(request) {
        ...
        try {
            ...
            const mockResponse = {
                ...
                cost: this.calculateCost({ totalTokens: request.prompt.length / 4 + 40 }, 0.0000015),
    ...
    async *generateStream(request) {
        ...
        for (let i = 0; i < workflowSteps.length; i++) {
            yield {
                ...
                finishReason: i === workflowSteps.length - 1 ? "workflow_completed" : undefined,
                metadata: {
                    step: i + 1,
    ...
    async validateRequest(request) {
        ...
        if (request.prompt.length > 500000) {
            throw this.createError("Workflow input exceeds maximum length", "INPUT_TOO_LONG");
        }
        return true;
    }
    ...
    handleError(error, _request) {
        const adapterError = this.createError(error.message || "Jules workflow error", error.code || "WORKFLOW_ERROR", error.status || 500, error.code === "WORKFLOW_BUSY" || error.code === "RATE_LIMIT");
        throw adapterError;
    }
}
//# sourceMappingURL=jules-workflow-adapter.js.map