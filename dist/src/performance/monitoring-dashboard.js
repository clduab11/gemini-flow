...
export class MonitoringDashboard extends EventEmitter {
    ...
    recordMetric(metric) {
        ...
        this.metrics.set(metric.id, metricHistory.filter((m) => m.timestamp > cutoffTime));
     ...
     */
    enableRealTimeStreaming(dashboardId) {
        const dashboard = this.dashboards.get(dashboardId);
        if (!dashboard) {
            throw new Error(`Dashboard ${dashboardId} not found`);
        }
    ...
    importDashboard(config) {
        const dashboard = JSON.parse(config);
     ...
     */
    getMetricStatistics(metricId, timeRange) {
        const metrics = this.metrics.get(metricId) || [];
        const filteredMetrics = metrics.filter((m) => m.timestamp >= timeRange.start && m.timestamp <= timeRange.end);
    ...
    evaluateWidgetStatus(widget, data) {
    ...
    calculateOverallHealth(components, performance, resources) {
        const criticalComponents = Array.from(components.values()).filter((c) => c.status === "critical").length;
        if (criticalComponents > 0 || performance.availability < 99.0) {
            return "critical";
        }
        const warningComponents = Array.from(components.values()).filter((c) => c.status === "warning").length;
        if (warningComponents > 0 || performance.errorRate > 0.05) {
            return "warning";
        }
        return "healthy";
    }
    getMetricsInRange(timeRange) {
        const allMetrics = [];
        for (const metricList of this.metrics.values()) {
            const filteredMetrics = metricList.filter((m) => m.timestamp >= timeRange.start && m.timestamp <= timeRange.end);
...
class AlertEngine {
    ...
    evaluateMetric(metric, alerts) {
        // Evaluate if metric triggers any alerts
    }
    evaluateAllAlerts(metrics, alerts) {
