...
export class A2ATransportLayer extends EventEmitter {
    ...
    async connect(agentId, config) {
        ...
        if (!this.isProtocolSupported(config.protocol)) {
            throw this.createTransportError("protocol_error", `Unsupported protocol: ${config.protocol}`);
        }
        if (!this.connectionPool.hasCapacity(agentId)) {
            throw this.createTransportError("resource_exhausted", "Connection pool capacity exceeded");
        }
        try {
     ...
     */
    async sendNotification(connectionId, notification) {
        const connection = this.connectionPool.getConnection(connectionId);
        if (!connection) {
            throw this.createTransportError("routing_error", `Connection not found: ${connectionId}`);
        }
     ...
     */
    async broadcastMessage(message, excludeConnections) {
     ...
     */
    getTransportMetrics() {
        const activeConnections = Array.from(this.connectionPool.getAllConnections().values()).filter((conn) => conn.isConnected).length;
     ...
     * Initialize protocol-specific handlers
     */
    async initializeProtocols(configs) {
     ...
     */
    async establishConnection(agentId, config) {
     ...
     */
    async establishTcpConnection(connection) {
        const config = connection.config;
     ...
     */
    async sendMessageInternal(connection, message) {
     ...
     */
    async sendWebSocketMessage(connection, message) {
     ...
     * Send gRPC message
     */
    async sendGrpcMessage(connection, message) {
     ...
     * Send TCP message
     */
    async sendTcpMessage(connection, message) {
                ...
                try {
                    ...
                    for (const frameData of frames) {
                        if (frameData.type === "response") {
                            const response = JSON.parse(frameData.payload.toString());
                            if (response.id === messageId) {
     ...
     */
    async sendNotificationInternal(connection, notification) {
     ...
     */
    async handleAuthentication(connection) {
     ...
     */
    async handleTlsConfiguration(connection) {
     ...
     */
    trackMessageSuccess(protocol, latency, message, response) {
     ...
     */
    trackMessageFailure(protocol, error) {
     ...
     */
    createTransportError(type, message) {
        return {
     ...
     */
    async simulateConnectionDelay(config) {
     ...
     */
    createMessageFrame(type, payload) {
        const version = 1;
        const flags = 0;
     ...
     */
    setupWebSocketListeners(connection, ws) {
        ws.on("close", () => {
     ...
     */
    setupHttp2Listeners(connection, session) {
        session.on("close", () => {
            this.handleConnectionClose(connection);
        });
        session.on("error", (error) => {
     ...
     */
    setupTcpListeners(connection, socket) {
        socket.on("close", () => {
            this.handleConnectionClose(connection);
        });
        socket.on("error", (error) => {
    ...
    scheduleReconnection(connection) {
        ...
        const delay = Math.min(1000 * Math.pow(state.backoffMultiplier, state.reconnectAttempts - 1), 30000);
        setTimeout(async () => {
            try {
     ...
     * Initialize connection state for reconnection
     */
    initializeConnectionState(connectionId) {
...
//# sourceMappingURL=a2a-transport-layer.js.map