...
class ConsistentHashRing {
    ring = new Map(); // hash -> nodeId
    virtualNodes = new Map(); // nodeId -> hashes
    virtualNodeCount;
    ...
    addNode(nodeId) {
        ...
        this.virtualNodes.set(nodeId, hashes.sort((a, b) => a - b));
        this.sortRing();
    }
    removeNode(nodeId) {
    ...
    sortRing() {
        const sorted = new Map(Array.from(this.ring.entries()).sort(([a], [b]) => a - b));
        this.ring = sorted;
    }
}
/**
...
export class MemorySharding extends EventEmitter {
    ...
    constructor(strategy = "consistent_hash", config = {}) {
        super();
    ...
    addNode(nodeId, capacity) {
        ...
        if (this.config.strategy === "consistent_hash" ||
     ...
     * Create a new shard
     */
    createShard(startKey, endKey, nodeId, replicas = []) {
        const shard = {
     ...
     */
    async splitShard(shardId, splitKey) {
        const shard = this.shardMap.shards.get(shardId);
        if (!shard) {
            throw new Error(`Shard not found: ${shardId}`);
        }
        ...
        try {
            ...
            const leftShard = this.createShard(shard.startKey, splitKey, shard.nodeId, shard.replicas);
            const rightShard = this.createShard(splitKey, shard.endKey, shard.nodeId, shard.replicas);
            // Create migration tasks
            const leftMigration = this.createMigrationTask("split", shardId, leftShard.shardId, shard.nodeId, leftShard.nodeId, { start: shard.startKey, end: splitKey });
            const rightMigration = this.createMigrationTask("split", shardId, rightShard.shardId, shard.nodeId, rightShard.nodeId, { start: splitKey, end: shard.endKey });
            // Execute migrations
            await Promise.all([
     ...
     */
    async mergeShards(shard1Id, shard2Id) {
        ...
        const [leftShard, rightShard] = shard1.endKey === shard2.startKey ? [shard1, shard2] : [shard2, shard1];
        // Mark shards as merging
        leftShard.status = "merging";
        rightShard.status = "merging";
        try {
            // Create merged shard
            const mergedShard = this.createShard(leftShard.startKey, rightShard.endKey, leftShard.nodeId, // Use left shard's node
            leftShard.replicas);
            // Create migration tasks
            const leftMigration = this.createMigrationTask("merge", leftShard.shardId, mergedShard.shardId, leftShard.nodeId, mergedShard.nodeId, { start: leftShard.startKey, end: leftShard.endKey });
            const rightMigration = this.createMigrationTask("merge", rightShard.shardId, mergedShard.shardId, rightShard.nodeId, mergedShard.nodeId, { start: rightShard.startKey, end: rightShard.endKey });
            // Execute migrations
            await Promise.all([
                this.executeMigration(leftMigration),
                this.executeMigration(rightMigration),
    ...
    keyInRange(key, startKey, endKey) {
        return key >= startKey && key < endKey;
    }
    createShardForKey(key, nodeId) {
    ...
    selectReplicaNodes(primaryNodeId) {
        const replicas = [];
        const availableNodes = Array.from(this.shardMap.nodeAssignments.keys()).filter((nodeId) => nodeId !== primaryNodeId);
        ...
        const replicaCount = Math.min(this.config.replicationFactor - 1, availableNodes.length);
        for (let i = 0; i < replicaCount; i++) {
            replicas.push(availableNodes[i]);
        }
        return replicas;
    }
    addShardToNode(nodeId, shardId) {
    ...
    removeShardFromNode(nodeId, shardId) {
        const currentShards = this.shardMap.nodeAssignments.get(nodeId) || [];
    ...
    migrateShards(shardIds, fromNodeId) {
        for (const shardId of shardIds) {
            const targetNodeId = this.selectNodeForNewShard();
            if (targetNodeId && targetNodeId !== fromNodeId) {
                const shard = this.shardMap.shards.get(shardId);
                if (shard) {
                    const migration = this.createMigrationTask("move", shardId, undefined, fromNodeId, targetNodeId, { start: shard.startKey, end: shard.endKey });
                    this.executeMigration(migration);
                }
            }
        }
    }
    createMigrationTask(type, sourceShardId, targetShardId, sourceNodeId, targetNodeId, keyRange) {
        const task = {
    ...
    calculateImbalance() {
        ...
        const maxDeviation = Math.max(...nodeLoads.map((load) => Math.abs(load - avgLoad)));
        return avgLoad > 0 ? maxDeviation / avgLoad : 0;
    }
    createRebalancePlan(nodes) {
        const plan = [];
        ...
        const avgLoad = Array.from(nodeLoads.values()).reduce((a, b) => a + b, 0) /
            nodeLoads.size;
        // Find overloaded and underloaded nodes
        const overloaded = Array.from(nodeLoads.entries())
            .filter(([, load]) => load > avgLoad * (1 + this.config.rebalanceThreshold))
            .sort(([, a], [, b]) => b - a);
        ...
        for (const [overloadedNode] of overloaded) {
            ...
            for (const shardId of shardIds) {
                ...
                if (shard && shard.status === "active") {
                    ...
                    const migration = this.createMigrationTask("move", shardId, undefined, overloadedNode, underloadedNode, { start: shard.startKey, end: shard.endKey });
                    plan.push(migration);
    ...
    updateMetrics() {
        ...
        const totalCapacity = Array.from(this.shardMap.nodeAssignments.keys()).length *
    ...
    generateStartKey(key) {
        // Generate a start key that's slightly before the given key
        return (key.slice(0, -1) + String.fromCharCode(key.charCodeAt(key.length - 1) - 1));
    }
    generateEndKey(key) {
...
//# sourceMappingURL=memory-sharding.js.map