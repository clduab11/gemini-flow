...
export class ConflictResolver extends EventEmitter {
    ...
    constructor(vectorClock, defaultStrategy = "lww") {
        super();
     ...
     */
    async resolve(localValue, remoteValue, key = "unknown", namespace = "default") {
        ...
        try {
            ...
            const context = {
                ...
                metadata: {
                    ...
                    timestamp: new Date(),
                    priority: Math.max(localValue.metadata?.priority || 5, remoteValue.metadata?.priority || 5),
                },
            };
     ...
     */
    registerCustomResolver(name, resolver) {
     ...
     */
    async semanticMerge(localValue, remoteValue, schema) {
        try {
            ...
            return this.vectorClock.compare(new VectorClock("local"), new VectorClock("remote")) === "after"
                ? localValue
                : remoteValue;
        }
        catch (error) {
     ...
     */
    async applyOperationalTransforms(baseText, transforms) {
     ...
     * Private methods
     */
    initializeDefaultRules(defaultStrategy) {
    ...
    initializeBuiltinResolvers() {
        ...
        this.customResolvers.set("lww", async (context) => {
            const comparison = context.localValue.vectorClock.compare(context.remoteValue.vectorClock);
            let resolvedValue;
            let confidence = 0.9;
            if (comparison === "after") {
    ...
    determineConflictType(localValue, remoteValue) {
    ...
    async findCommonAncestor(localValue, remoteValue) {
    ...
    evaluateCondition(condition, context) {
        ...
        if (condition.field.startsWith("local.")) {
            value = this.getNestedValue(context.localValue, condition.field.substring(6));
        }
        else if (condition.field.startsWith("remote.")) {
            value = this.getNestedValue(context.remoteValue, condition.field.substring(7));
        }
        else {
    ...
    async executeResolutionStrategy(context, rule) {
    ...
    async executeSemanticResolution(context) {
        try {
            const mergedValue = await this.semanticMerge(context.localValue.data, context.remoteValue.data);
            return {
                resolutionId: this.generateResolutionId(),
                strategy: "semantic",
    ...
    async executeOperationalResolution(context) {
        if (typeof context.localValue.data !== "string" ||
        ...
        const transforms = this.generateOperationalTransforms(context.commonAncestor?.data || "", context.localValue.data, context.remoteValue.data);
        // Apply transforms to base text
        const baseText = context.commonAncestor?.data || context.localValue.data;
        const resolvedText = await this.applyOperationalTransforms(baseText, transforms);
        return {
            resolutionId: this.generateResolutionId(),
            strategy: "operational",
    ...
    async executeIntersectionResolution(context) {
        let resolvedValue;
        if (Array.isArray(context.localValue.data) &&
            Array.isArray(context.remoteValue.data)) {
            resolvedValue = context.localValue.data.filter((item) => context.remoteValue.data.includes(item));
        }
        else if (this.isSet(context.localValue.data) &&
            this.isSet(context.remoteValue.data)) {
            resolvedValue = new Set([...context.localValue.data].filter((item) => context.remoteValue.data.has(item)));
        }
        else {
        ...
        return {
            ...
            requiresManualReview: Array.isArray(resolvedValue) && resolvedValue.length === 0,
            timestamp: new Date(),
            resolverAgent: this.vectorClock.getAgents()[0] || "system",
        };
    }
    generateOperationalTransforms(baseText, localText, remoteText) {
        const transforms = [];
    ...
    applyTransform(text, transform, offset) {
    ...
    calculateOffset(transform, currentOffset) {
        switch (transform.type) {
            case "insert":
    ...
    mergeObjects(local, remote, schema) {
        const merged = { ...local };
        for (const [key, value] of Object.entries(remote)) {
            if (!(key in merged)) {
                ...
                merged[key] = this.resolveObjectConflict(key, merged[key], value, schema);
            }
        }
        return merged;
    }
    mergeStrings(local, remote) {
    ...
    mergeNumbers(local, remote) {
        // Average of numbers (simple strategy)
        return (local + remote) / 2;
    }
    resolveObjectConflict(key, localValue, remoteValue, schema) {
    ...
    createFallbackResolution(localValue, remoteValue) {
        return {
            resolutionId: this.generateResolutionId(),
            strategy: "lww",
    ...
    updateStats(resolution, resolutionTime) {
        // Update strategy statistics
        const strategyCount = this.stats.resolutionsByStrategy.get(resolution.strategy) || 0;
        this.stats.resolutionsByStrategy.set(resolution.strategy, strategyCount + 1);
    ...
    hasStructuralDifferences(local, remote) {
    ...
    hasSemanticConflicts(local, remote) {
