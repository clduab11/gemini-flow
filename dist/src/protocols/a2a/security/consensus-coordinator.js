...
export class ConsensusDetectionCoordinator extends EventEmitter {
    ...
    activeConsensus = new Map(); // targetAgentId -> consensus
    completedConsensus = [];
    evidencePool = new Map(); // evidenceId -> evidence
    // Voter management
    eligibleVoters = new Set();
    voterReputations = new Map();
    voterStakes = new Map();
    // Cryptographic infrastructure
    consensusKeys = new Map();
    thresholdScheme;
    // Anti-collusion and privacy
    commitStore = new Map(); // voteId -> commit
    blindingFactors = new Map();
    constructor(config = {}) {
        super();
     ...
     */
    async registerVoter(voterId, reputation, stake = 0) {
     ...
     */
    async initiateConsensus(targetAgentId, initialEvidence, detectionResult) {
     ...
     */
    async executeRound1(consensus, initialEvidence) {
        const round = {
     ...
     */
    async submitEvidence(consensusId, evidence) {
     ...
     */
    async submitVote(consensusId, vote) {
        const consensus = this.findConsensusById(consensusId);
        if (!consensus) {
            throw new Error("Consensus not found");
        }
     ...
     */
    async processRound1Timeout(consensus, round) {
     ...
     */
    async executeRound2(consensus, aggregatedEvidence) {
        const round = {
            roundId: crypto.randomUUID(),
            targetAgentId: consensus.targetAgentId,
            roundNumber: 2,
     ...
     */
    async processRound2Timeout(consensus, round) {
        if (round.status !== "active")
            return;
        // Check participation
        const participationRate = round.actualVoters.length / round.eligibleVoters.length;
     ...
     */
    async processRound3Timeout(consensus, round) {
        if (round.status !== "active")
            return;
     ...
     */
    async finalizeConsensus(consensus) {
        ...
        if (this.config.thresholdSignatures) {
            consensus.thresholdSignature =
                await this.thresholdScheme.generateSignature(consensus.consensusHash, consensus.rounds.flatMap((r) => r.actualVoters));
        }
     ...
     */
    async checkRoundCompletion(consensus, round) {
        const participationRate = round.actualVoters.length / round.eligibleVoters.length;
    ...
    calculateVoteWeight(reputation, stake) {
    ...
    async makeFinalDecision(consensus) {
        const rounds = consensus.rounds;
    ...
    async calculateFinalMetrics(consensus) {
        // Aggregate vote counts
        consensus.totalVotes = consensus.rounds.reduce((sum, r) => sum + r.votes.length, 0);
        consensus.maliciousVotes = consensus.rounds.reduce((sum, r) => sum + r.votes.filter((v) => v.decision === "malicious").length, 0);
        consensus.benignVotes = consensus.rounds.reduce((sum, r) => sum + r.votes.filter((v) => v.decision === "benign").length, 0);
        consensus.abstentions = consensus.rounds.reduce((sum, r) => sum + r.votes.filter((v) => v.decision === "abstain").length, 0);
     ...
     */
    calculateRoundHash(roundNumber, targetAgentId, evidence) {
    ...
    async verifyVote(vote, round) {
        // Verify vote signature and constraints
        return (vote.confidence >= 0 &&
            vote.confidence <= 1 &&
            ["malicious", "benign", "abstain"].includes(vote.decision));
    }
    async generateConsensusHash(consensus) {
        const data = {
            consensusId: consensus.consensusId,
            targetAgentId: consensus.targetAgentId,
            finalDecision: consensus.finalDecision,
    ...
    async verifyConsensusIntegrity(consensus) {
    ...
    async aggregateEvidence(targetAgentId) {
        const relevantEvidence = Array.from(this.evidencePool.values()).filter((e) => e.data.witnessStatements?.some((w) => w.witnessId) ||
            e.data.behaviorDeviations?.length > 0);
        return {
            evidenceId: crypto.randomUUID(),
            sourceRound: 1,
            type: "aggregated",
            weight: relevantEvidence.reduce((sum, e) => sum + e.weight, 0) /
                relevantEvidence.length || 0,
            reliability: relevantEvidence.reduce((sum, e) => sum + e.reliability, 0) /
                relevantEvidence.length || 0,
            data: {
    ...
    async aggregateAllEvidence(targetAgentId) {
        return await this.aggregateEvidence(targetAgentId);
    }
    async revealRound1Votes(consensus) {
    ...
    getCompletedConsensus() {
        return this.completedConsensus;
    }
    getConsensusById(consensusId) {
    ...
    calculateAverageConsensusTime() {
        if (this.completedConsensus.length === 0)
            return 0;
...
class ThresholdSignatureScheme {
    ...
    async generateSignature(message, signers) {
...
//# sourceMappingURL=consensus-coordinator.js.map