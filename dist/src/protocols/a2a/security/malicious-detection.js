...
export class MaliciousAgentDetector extends EventEmitter {
    ...
    consensusVotes = new Map(); // targetAgentId -> votes
     ...
     */
    async recordAgentBehavior(agentId, message, identity, metadata = {}) {
        try {
     ...
     */
    async performDetectionRound() {
        try {
            const activeAgents = Array.from(this.behaviorProfiles.keys()).filter((agentId) => !this.quarantinedAgents.has(agentId));
     ...
     */
    async analyzeAgentBehavior(agentId) {
        ...
        const mlConfidence = mlAnomalies.reduce((sum, a) => sum + a.confidence, 0) /
     ...
     */
    async evaluateDetectionPattern(profile, pattern) {
        const evidence = [];
        let totalWeight = 0;
        let matchedWeight = 0;
     ...
     */
    async evaluateDetectionRule(profile, rule) {
     ...
     */
    extractMetricValue(profile, metricPath) {
     ...
     */
    async initiateConsensusDetection(detection) {
    ...
    async processConsensusVotes() {
        for (const [targetAgentId, votes] of this.consensusVotes) {
            ...
            if (currentRoundVotes.length < 3)
                continue; // Need minimum votes
            // Calculate consensus
            ...
            const avgConfidence = maliciousVotes.reduce((sum, v) => sum + v.confidence, 0) /
     ...
     */
    async quarantineAgent(agentId, detection) {
     ...
     */
    async processRecoveryResponse(agentId, response) {
        if (!this.quarantinedAgents.has(agentId)) {
            return false;
        }
     ...
     */
    createInitialBehaviorProfile(agentId, identity) {
        return {
            agentId,
    ...
    async updateBehaviorProfile(profile, message, metadata) {
        const now = Date.now();
        const payloadSize = JSON.stringify(message.payload).length;
        // Update message frequency
        profile.messageFrequency.perMinute = this.updateMovingAverage(profile.messageFrequency.perMinute, 1, 60);
        // Update message patterns
        profile.messagePatterns.avgPayloadSize = this.updateMovingAverage(profile.messagePatterns.avgPayloadSize, payloadSize, 100);
        ...
        if (metadata.resourceUsage) {
            profile.networkBehavior.resourceUsage = this.updateMovingAverage(profile.networkBehavior.resourceUsage, metadata.resourceUsage, 100);
        }
    }
    updateMovingAverage(current, newValue, windowSize) {
        return (current * (windowSize - 1) + newValue) / windowSize;
    }
    async detectRealTimeAnomalies(profile, message) {
        const anomalies = [];
    ...
    extractBehaviorDeviations(profile) {
        const deviations = [];
    ...
    extractConsensusViolations(profile) {
        const violations = [];
    ...
    extractProtocolViolations(profile) {
        const violations = [];
    ...
    extractNetworkAnomalies(profile) {
        const anomalies = [];
    ...
    determineRecommendedAction(confidence) {
        if (confidence >= 0.9)
            return "ban";
        if (confidence >= 0.8)
            return "quarantine";
    ...
    async generateRecoveryChallenge(agentId) {
    ...
    async validateRecoveryResponse(agentId, response) {
     ...
     * Public API methods
     */
    getBehaviorProfile(agentId) {
        return this.behaviorProfiles.get(agentId) || null;
    }
    getDetectionResult(agentId) {
...
class BehaviorModel {
    model; // Placeholder for actual ML model
