...
export class IntelligentCacheManager extends EventEmitter {
    ...
    async performPredictivePrefetch(key, context) {
    ...
    async predictRelatedKeys(key, context) {
    ...
    async generateKeysFromPattern(pattern, context) {
...
export class ConnectionPoolManager extends EventEmitter {
     ...
     */
    async acquireConnection(toolId, timeout = 5000) {
     ...
     */
    async releaseConnection(toolId, connection) {
    ...
    async performHealthChecks() {
        const healthCheckPromises = Array.from(this.pools.values()).map((pool) => pool.performHealthCheck().catch((error) => {
...
export class CachingConnectionPoolService extends EventEmitter {
    ...
    connectionPool;
    logger;
    constructor(cacheConfig, poolConfig) {
        super();
     ...
     */
    async executeWithOptimizations(invocation, executor) {
        const cacheKey = this.generateCacheKey(invocation);
        // Try cache first
        const cached = await this.cache.get(cacheKey, invocation.context);
        if (cached) {
            return cached;
        }
...
//# sourceMappingURL=caching-connection-pool.js.map