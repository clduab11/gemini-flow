...
export class WebAgentCoordinator extends BaseIntegration {
    orchestrator;
    siteCache = new Map();
    ...
    async initialize() {
        try {
            ...
            this.status = "error";
            const coordinatorError = new IntegrationBaseError(`Failed to initialize Web Agent Coordinator: ${error.message}`, "INIT_FAILED", "WebAgentCoordinator", "critical", false, { originalError: error.message });
        ...
        try {
            ...
            switch (navigation.strategy) {
                ...
                case "adaptive":
                    ...
                    break;
                default:
                    throw new Error(`Unknown navigation strategy: ${navigation.strategy}`);
            }
            ...
            const navigationError = new IntegrationBaseError(`Site navigation failed: ${error.message}`, "NAVIGATION_FAILED", "WebAgentCoordinator", "medium", true, { url: navigation.url, strategy: navigation.strategy });
        ...
        try {
            ...
            switch (workflow.coordination.strategy) {
                ...
                case "hybrid":
                    ...
                    break;
                default:
                    throw new Error(`Unknown workflow strategy: ${workflow.coordination.strategy}`);
            }
            ...
            if (workflow.validation.finalValidation) {
                const validationPassed = workflow.validation.finalValidation(Object.fromEntries(results));
                if (!validationPassed) {
            ...
            const duration = performance.now() - startTime;
            const workflowError = new IntegrationBaseError(`Workflow execution failed: ${error.message}`, "WORKFLOW_FAILED", "WebAgentCoordinator", "high", true, { workflowId: workflow.id, workflowName: workflow.name });
    ...
    async coordiateMultiSite(sites, action) {
        const startTime = performance.now();
        try {
            ...
            switch (action.coordination.strategy) {
                ...
                case "adaptive":
                    ...
                    break;
                default:
                    throw new Error(`Unknown multi-site strategy: ${action.coordination.strategy}`);
            }
            // Aggregate results
            ...
            const duration = performance.now() - startTime;
            const multiSiteError = new IntegrationBaseError(`Multi-site coordination failed: ${error.message}`, "MULTISITE_FAILED", "WebAgentCoordinator", "high", true, { actionId: action.id, sitesCount: sites.length });
    ...
    async optimizeNavigation(pattern) {
        try {
            ...
            if (!pattern.optimization.preloadResources.length) {
                ...
                implementationPlan.push('Add <link rel="preload"> tags for critical resources');
            }
            ...
            if (pattern.optimization.cacheStrategy === "none") {
                ...
                implementationPlan.push("Implement cache-first strategy for static assets");
            }
            ...
            pattern.optimization = {
                preloadResources: pattern.optimization.preloadResources.length > 0
                    ? pattern.optimization.preloadResources
                    : ["style.css", "app.js", "fonts.woff2"],
                cacheStrategy: pattern.optimization.cacheStrategy !== "none"
            ...
            this.logger.info("Navigation optimization completed", {
                ...
                expectedGain: Object.values(performanceGains).reduce((a, b) => a + b, 0),
            });
            ...
            const optimizationError = new IntegrationBaseError(`Navigation optimization failed: ${error.message}`, "OPTIMIZATION_FAILED", "WebAgentCoordinator", "low", true, { sitesCount: pattern.sites.length });
            this.emitError(optimizationError);
            throw optimizationError;
        }
    }
    async learnSiteStructure(url) {
        try {
            this.logger.info("Learning site structure", { url });
            // Check cache first
            if (this.siteCache.has(url)) {
                const cached = this.siteCache.get(url);
                if (Date.now() - cached.metadata.lastScanned.getTime() <
                    24 * 60 * 60 * 1000) {
            ...
            const learningError = new IntegrationBaseError(`Site learning failed: ${error.message}`, "SITE_LEARNING_FAILED", "WebAgentCoordinator", "medium", true, { url });
            this.emitError(learningError);
            throw learningError;
        }
    }
    // === PRIVATE HELPER METHODS ===
    async executeDirectNavigation(tab, navigation) {
        const startTime = performance.now();
        const errors = [];
        ...
        try {
            ...
            for (const fallbackUrl of navigation.fallbackOptions) {
                try {
                    ...
                    errors.push(`Fallback failed: ${fallbackUrl} - ${fallbackError.message}`);
                }
            }
            throw new Error(`Direct navigation failed: ${error.message}`);
        }
    }
    async executeProgressiveNavigation(tab, navigation) {
        const startTime = performance.now();
        const errors = [];
        let checkpointsPassed = 0;
        try {
    ...
    async executeIntelligentNavigation(tab, navigation) {
        const startTime = performance.now();
        const errors = [];
        let checkpointsPassed = 0;
        try {
            ...
            if (siteStructure) {
                ...
                await this.navigateUsingStructure(tab.page, navigation.url, siteStructure);
            }
            else {
            ...
            for (const checkpoint of navigation.checkpoints) {
                try {
                    await this.executeIntelligentCheckpoint(tab.page, checkpoint, siteStructure);
                    checkpointsPassed++;
                }
                catch (error) {
                    if (checkpoint.required) {
    ...
    async executeAdaptiveNavigation(tab, navigation) {
        const startTime = performance.now();
        const errors = [];
        let checkpointsPassed = 0;
        try {
            ...
            const connection = await tab.page.evaluate(() => navigator.connection?.effectiveType || "unknown");
            ...
            switch (currentStrategy) {
                case "progressive":
                    return await this.executeProgressiveNavigation(tab, adaptedNavigation);
                case "intelligent":
                    return await this.executeIntelligentNavigation(tab, adaptedNavigation);
                default:
    ...
    async executeCheckpoint(page, checkpoint) {
        switch (checkpoint.action) {
            ...
            case "verify":
                ...
                if (!element) {
                    throw new Error(`Verification failed: ${checkpoint.selector} not found`);
                }
                break;
            case "extract":
                const content = await page.$eval(checkpoint.selector, (el) => el.textContent);
                if (!content) {
                    throw new Error(`Extraction failed: ${checkpoint.selector} has no content`);
                }
                break;
            default:
    ...
    async executeIntelligentCheckpoint(page, checkpoint, structure) {
    ...
    async navigateUsingStructure(page, url, structure) {
    ...
    async executeSequentialWorkflow(workflow, results, errors) {
            ...
            try {
                ...
                if (step.retryPolicy.maxAttempts > 1) {
                    ...
                    for (let attempt = 1; attempt < step.retryPolicy.maxAttempts; attempt++) {
                        try {
                            await new Promise((resolve) => setTimeout(resolve, step.retryPolicy.backoffMs * attempt));
                            const result = await this.executeWorkflowStep(step);
                            results.set(step.id, result);
                            break;
                        }
                            ...
                            if (attempt === step.retryPolicy.maxAttempts - 1) {
                                errors.push(`Step ${step.id} failed after ${attempt + 1} attempts`);
                            }
                        }
                    }
                }
    ...
    async executeParallelWorkflow(workflow, results, errors) {
    ...
    async executeHybridWorkflow(workflow, results, errors) {
        // Group steps by dependencies
        const independentSteps = workflow.steps.filter((step) => step.dependencies.length === 0);
        const dependentSteps = workflow.steps.filter((step) => step.dependencies.length > 0);
        // Execute independent steps in parallel
        await this.executeParallelWorkflow({ ...workflow, steps: independentSteps }, results, errors);
        // Execute dependent steps sequentially
        await this.executeSequentialWorkflow({ ...workflow, steps: dependentSteps }, results, errors);
    }
    ...
    async executeSequentialMultiSite(action, results) {
    ...
    async executeAdaptiveMultiSite(action, results) {
        ...
        const normalPrioritySites = sortedSites.filter((site) => site.priority <= 7);
    ...
    async analyzeSiteStructure(page, url) {
...
//# sourceMappingURL=web-agent-coordinator.js.map