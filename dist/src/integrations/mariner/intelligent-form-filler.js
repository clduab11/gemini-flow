...
export class IntelligentFormFiller extends BaseIntegration {
    ...
    async initialize() {
        try {
            ...
            this.status = "error";
            const fillerError = new IntegrationBaseError(`Failed to initialize Intelligent Form Filler: ${error.message}`, "INIT_FAILED", "IntelligentFormFiller", "critical", false, { originalError: error.message });
    ...
    async fillForm(request) {
        ...
        try {
            ...
            const fillResult = await this.performIntelligentFilling(request.tab, fieldMappings, request.options);
            // Validate form if required
            let validationErrors = [];
            if (request.validation.validateBeforeSubmit) {
                validationErrors = await this.validateForm(request.tab, formStructure, request.validation);
            }
            ...
            const duration = performance.now() - startTime;
            const fillingError = new IntegrationBaseError(`Form filling failed: ${error.message}`, "FORM_FILLING_FAILED", "IntelligentFormFiller", "medium", true, { requestId: request.id, tabId: request.tab.id });
    ...
    // === FORM ANALYSIS ===
    async analyzeFormStructure(tab) {
        try {
            const page = tab.page;
            const formAnalysis = await page.evaluate(() => {
                ...
                return {
                    ...
                    submission: {
                        selector: this.generateSelector(form.querySelector('[type="submit"], button[type="submit"], input[type="submit"]') || form.querySelector("button")),
                        method: "click",
            ...
            throw new IntegrationBaseError(`Form analysis failed: ${error.message}`, "FORM_ANALYSIS_FAILED", "IntelligentFormFiller", "medium", true, { tabId: tab.id, url: tab.url });
        }
    }
    // === DATA MAPPING ===
    mapFormData(formData, formStructure) {
    ...
    smartMatchField(field, formData) {
        ...
        if (fieldLower.includes("name") ||
            ...
            if (fieldLower.includes("first")) {
                return this.findValueByPattern(formData, /first.*name|given.*name|fname/i);
            }
            if (fieldLower.includes("last")) {
                return this.findValueByPattern(formData, /last.*name|family.*name|surname|lname/i);
            }
            return this.findValueByPattern(formData, /^name$|full.*name|display.*name/i);
        }
        // Phone fields
    ...
    findValueByPattern(data, pattern) {
    ...
    // === INTELLIGENT FILLING ===
    async performIntelligentFilling(tab, fieldMappings, options) {
        const page = tab.page;
    ...
    async fillFieldByType(page, selector, value, fieldInfo) {
    ...
    async fillInputField(page, selector, value, type) {
        switch (type) {
            case "checkbox":
            case "radio":
                if (value) {
    ...
    async fillSelectField(page, selector, value) {
            ...
            await page.evaluate((sel, val) => {
                ...
                if (select) {
                    for (let i = 0; i < select.options.length; i++) {
                        if (select.options[i].text.toLowerCase().includes(val.toLowerCase())) {
    ...
    async fillGenericField(page, selector, value) {
    ...
    async tryAlternativeFilling(page, selector, value) {
    ...
    // === VALIDATION ===
    async validateForm(tab, formStructure, validation) {
        const errors = [];
        const page = tab.page;
        try {
            ...
            for (const rule of this.config.validationRules) {
                ...
                if (field) {
                    ...
                    if (!this.validateFieldValue(value, rule)) {
                        errors.push(`Field '${rule.field}' failed validation: ${rule.type}`);
                    }
                }
            }
    ...
    // === FORM SUBMISSION ===
    async submitForm(tab, formStructure) {
        const page = tab.page;
        try {
    ...
    async handleCaptcha(page) {
        ...
        try {
            const captchaElements = await page.$$eval('[class*="captcha"], [id*="captcha"], [src*="captcha"]', (elements) => elements.length);
    ...
    initializeFieldMappings() {
        ...
        const commonMappings = [
            ...
            [
                ...
                {
                    sourceField: "address",
                    targetSelector: '[name*="address"], [id*="address"], [name*="street"]',
                    priority: 7,
                },
            ],
        ];
...
//# sourceMappingURL=intelligent-form-filler.js.map