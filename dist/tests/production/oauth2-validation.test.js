...
describe('OAuth2 Production Validation', () => {
    ...
    describe('Token Exchange Validation', () => {
        it('should handle invalid authorization code exchange', async () => {
            const result = await oauth2Provider.exchangeCodeForTokens('invalid-code', 'test-state', 'test-verifier');
        ...
        it('should validate state parameter for CSRF protection', async () => {
            ...
            const result = await oauth2Provider.exchangeCodeForTokens('test-code', 'invalid-state', 'test-verifier');
            expect(result.success).toBe(false);
...
//# sourceMappingURL=oauth2-validation.test.js.map