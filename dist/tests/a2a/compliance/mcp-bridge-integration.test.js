...
class MCPBridgeIntegrationTestSuite extends A2AComplianceTestSuite {
    ...
    async setupBridgeAgents() {
        // Create specialized bridge agents for MCP tool execution
        const bridgeAgent1 = A2ATestDataBuilder.createAgent('mcp-bridge-1', 'bridge', ['mcp', 'tool-execution', 'state-management'], []);
        const bridgeAgent2 = A2ATestDataBuilder.createAgent('mcp-bridge-2', 'bridge', ['mcp', 'tool-execution', 'performance-monitoring'], []);
        const bridgeAgent3 = A2ATestDataBuilder.createAgent('mcp-bridge-3', 'bridge', ['mcp', 'tool-execution', 'error-handling'], []);
    ...
    async registerToolsWithAgents(tools) {
    ...
    async executeBulkTools(tools, options = {}) {
        const results = {
    ...
    async executeConcurrentTools(tools, concurrency) {
        const startTime = Date.now();
        const promises = [];
        for (const toolName of tools) {
            const message = A2ATestDataBuilder.createMessage({
    ...
    generateMockParameters(toolName) {
    ...
    async executeMockMCPTool(toolName, params) {
...
//# sourceMappingURL=mcp-bridge-integration.test.js.map