...
describe('Authentication Flows E2E', () => {
    ...
    describe('OAuth2 Complete Flow', () => {
        ...
        it('should complete OAuth2 authentication flow end-to-end', async () => {
            ...
            const tokenResult = await oauth2Provider.exchangeCodeForTokens('mock-auth-code', authResult.context?.credentials?.metadata?.state, authResult.context?.credentials?.metadata?.pkceVerifier);
    ...
    describe('Multi-Provider Scenarios', () => {
        ...
        it('should manage provider priorities correctly', async () => {
            ...
            authManager.registerProvider('test', mockProvider, {}, { priority: 1 });
            const updatedProviders = authManager.getAvailableProviders();
            expect(updatedProviders).toContain('test');
        });
    });
    describe('Session Management and Lifecycle', () => {
        ...
        it('should handle concurrent session operations safely', async () => {
            ...
            const refreshPromises = Array.from({ length: 5 }, () => authManager.refreshCredentials(sessionId));
    ...
    describe('Event Handling and Monitoring', () => {
        ...
        it('should emit authentication events correctly', async () => {
            ...
            expect(authEventSpy).toHaveBeenCalledWith(expect.objectContaining({
                type: 'authentication',
                provider: 'oauth2',
                success: true
            }));
    ...
    describe('Security and Error Resilience', () => {
        ...
        it('should handle network failures gracefully', async () => {
            ...
            mockFetch.mockImplementation(() => new Promise((_, reject) => setTimeout(() => reject(new Error('Network timeout')), 100)));
            const authResult = await authManager.authenticate('oauth2');
            expect(authResult.success).toBe(false);
    ...
    describe('Performance and Scalability', () => {
        it('should handle high-volume authentication requests', async () => {
            ...
            const authPromises = Array.from({ length: 20 }, (_, i) => authManager.authenticate(i % 2 === 0 ? 'oauth2' : 'vertex-ai'));
...
//# sourceMappingURL=authentication-flows.test.js.map