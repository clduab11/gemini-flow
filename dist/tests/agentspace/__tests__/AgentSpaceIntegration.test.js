...
import { initializeAgentSpace, deployFullAgentSwarm, shutdownAgentSpace, } from "../AgentSpaceInitializer.js";
...
describe("AgentSpace Integration Tests", () => {
    let memoryManager;
    let agentSpaceSystem;
    let mcpBridge;
    let streamingIntegration;
    ...
    describe("System Initialization", () => {
        ...
        it("should handle invalid configuration gracefully", async () => {
            const invalidConfig = {
                ...testConfig,
                maxAgents: -5, // Invalid
                spatialDimensions: null, // Invalid
            };
            await expect(initializeAgentSpace(invalidConfig, memoryManager)).rejects.toThrow();
        });
        it("should validate memory manager integration", async () => {
            ...
            await result.agentSpaceManager.memoryArchitecture.storeMemoryNode(testMemoryNode);
            const retrievedNodes = await result.agentSpaceManager.memoryArchitecture.queryMemoryBySpatialProximity({ x: 0, y: 0, z: 0 }, 10);
    ...
    describe("Agent Deployment and Management", () => {
        ...
        it("should deploy full agent swarm with optimal spatial arrangement", async () => {
            const result = await deployFullAgentSwarm(agentSpaceSystem.manager, "mesh");
        ...
        it("should handle agent workspace creation and management", async () => {
            const workspaceResult = await agentSpaceSystem.manager.createWorkspace("test-workspace", {
                maxMemoryMB: 256,
                maxCPUPercentage: 20,
            ...
            const retrievedWorkspace = await agentSpaceSystem.manager.getWorkspace("test-workspace");
        ...
        it("should enforce resource limits in workspaces", async () => {
            const limitedWorkspace = await agentSpaceSystem.manager.createWorkspace("limited-workspace", {
            ...
            const resourceUsage = agentSpaceSystem.resourceAllocator.getResourceUsage(limitedWorkspace.id);
    ...
    describe("MCP Bridge Integration", () => {
        beforeEach(async () => {
            ...
            mcpBridge = new MCPBridge(agentSpaceSystem.manager, agentSpaceSystem.performanceMonitor, agentSpaceSystem.resourceAllocator);
        });
        ...
        it("should spawn agent with spatial integration through MCP", async () => {
            ...
            const agentResult = await mcpBridge.spawnAgentWithSpatialIntegration(swarmResult.swarmId, {
                type: "researcher",
        ...
        it("should orchestrate task with spatial coordination", async () => {
            ...
            const taskResult = await mcpBridge.orchestrateTaskWithSpatialCoordination(swarmResult.swarmId, {
        ...
        it("should bridge memory operations with spatial context", async () => {
            const memoryResult = await mcpBridge.bridgeMemoryOperationWithSpatialContext("store", {
    ...
    describe("Streaming Integration", () => {
        ...
        beforeEach(async () => {
            ...
            streamingIntegration = new StreamingIntegration(streamingAPI, agentSpaceSystem.manager);
        });
        it("should create spatial streaming session", async () => {
            const workspace = await agentSpaceSystem.manager.createWorkspace("streaming-test");
            const sessionResult = await streamingIntegration.createSpatialStreamingSession({
        ...
        it("should start spatial video stream with 3D context", async () => {
            const workspace = await agentSpaceSystem.manager.createWorkspace("video-test");
            ...
            const videoResult = await streamingIntegration.startSpatialVideoStream(session.session.id, {
                source: "camera",
                perspective: "3d",
        ...
        it("should start spatial audio stream with 3D processing", async () => {
            const workspace = await agentSpaceSystem.manager.createWorkspace("audio-test");
            const session = await streamingIntegration.createSpatialStreamingSession({
                agentId: "audio-agent",
                workspaceId: workspace.id,
                sessionType: "audio",
            ...
            const audioResult = await streamingIntegration.startSpatialAudioStream(session.session.id, {
                source: "microphone",
                spatialProfile: {
                    distance: 30,
                    attenuation: 0.5,
            ...
            expect(audioResult.audioStream.spatialAudioProfile.directionality).toBe("cardioid");
        ...
        it("should create spatial collaboration zone", async () => {
            const workspace1 = await agentSpaceSystem.manager.createWorkspace("collab-1");
            const workspace2 = await agentSpaceSystem.manager.createWorkspace("collab-2");
            ...
            const zoneResult = await streamingIntegration.createSpatialCollaborationZone({
    ...
    describe("Security Integration", () => {
        let securityOptimization;
        let coScientistSecurity;
        beforeEach(async () => {
            if (!agentSpaceSystem) {
                const result = await initializeAgentSpace(testConfig, memoryManager);
                agentSpaceSystem = {
            ...
            coScientistSecurity = new CoScientistSecurityIntegration(securityOptimization);
            securityIntegration = new SecurityIntegration(agentSpaceSystem.manager, coScientistSecurity);
        });
        it("should create spatial security context", async () => {
            const workspace = await agentSpaceSystem.manager.createWorkspace("secure-workspace");
            const securityResult = await securityIntegration.createSpatialSecurityContext({
                workspaceId: workspace.id,
        ...
        it("should establish secure collaboration zone", async () => {
            const zoneResult = await securityIntegration.establishSecureCollaborationZone({
        ...
        it("should validate spatial access controls", async () => {
            const workspace = await agentSpaceSystem.manager.createWorkspace("access-test");
            ...
            const accessResult = await securityIntegration.validateSpatialAccess("requesting-agent", {
                type: "workspace",
                id: workspace.id,
        ...
        it("should generate comprehensive security report", async () => {
            const workspace = await agentSpaceSystem.manager.createWorkspace("report-test");
            await securityIntegration.createSpatialSecurityContext({
                workspaceId: workspace.id,
            ...
            const reportResult = await securityIntegration.generateSecurityReport("workspace", workspace.id, "comprehensive");
    ...
    describe("Performance and Scalability", () => {
        ...
        it("should handle concurrent workspace creation", async () => {
            ...
            const startTime = performance.now();
            const workspacePromises = Array.from({ length: concurrentOperations }, (_, i) => agentSpaceSystem.manager.createWorkspace(`concurrent-workspace-${i}`));
        ...
        it("should maintain performance under memory pressure", async () => {
            ...
            const storePromises = Array.from({ length: memoryNodes }, (_, i) => agentSpaceSystem.manager.memoryArchitecture.storeMemoryNode({
                id: `perf-test-node-${i}`,
                type: "agent_memory",
            ...
            // Query memory nodes
            const queryResults = await agentSpaceSystem.manager.memoryArchitecture.queryMemoryBySpatialProximity({ x: 5, y: 5, z: 0 }, 20);
        ...
        it("should scale spatial operations efficiently", async () => {
            const agentCount = 25;
            const positions = Array.from({ length: agentCount }, (_, i) => ({
                x: (i % 5) * 20,
                y: Math.floor(i / 5) * 20,
                z: 0,
            }));
            ...
            for (let i = 0; i < agentCount; i++) {
                const entityId = await agentSpaceSystem.manager.spatialFramework.registerEntity({
            ...
            const queryPromises = positions.map((pos) => agentSpaceSystem.manager.spatialFramework.queryNearbyEntities(pos, 30));
            ...
            const avgNearbyEntities = queryResults.reduce((sum, result) => sum + result.length, 0) /
                queryResults.length;
        ...
        it("should provide accurate performance metrics", async () => {
            ...
            const performanceMetrics = agentSpaceSystem.performanceMonitor.getCurrentMetrics();
    ...
    describe("Error Handling and Recovery", () => {
        ...
        it("should handle invalid workspace operations gracefully", async () => {
            ...
            const nonExistentWorkspace = await agentSpaceSystem.manager.getWorkspace("non-existent");
            expect(nonExistentWorkspace).toBeNull();
            // Test destroying non-existent workspace
            const destroyResult = await agentSpaceSystem.manager.destroyWorkspace("non-existent");
        ...
        it("should recover from spatial operation failures", async () => {
            ...
            await expect(agentSpaceSystem.manager.spatialFramework.registerEntity({
                id: "invalid-entity",
                type: "agent",
        ...
        it("should handle memory operation failures", async () => {
            ...
            await expect(agentSpaceSystem.manager.memoryArchitecture.storeMemoryNode(invalidNode)).rejects.toThrow();
        });
        it("should maintain system stability under error conditions", async () => {
            ...
            const healthAfterErrors = await agentSpaceSystem.manager.getSystemHealth();
            expect(healthAfterErrors.overallHealth.overall).toBeGreaterThan(0.5);
            // Should still be able to create new resources
            const newWorkspace = await agentSpaceSystem.manager.createWorkspace("post-error-workspace");
...
//# sourceMappingURL=AgentSpaceIntegration.test.js.map