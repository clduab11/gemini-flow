...
describe("A2AMultimediaProtocol", () => {
    ...
    describe("Session Management", () => {
        ...
        it("should create multimedia session successfully", async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith("Creating multimedia session", expect.objectContaining({
        ...
        it("should create different session types", async () => {
            // Arrange
            const sessionTypes = [
                { type: "request_response", participants: ["agent-002"] },
                {
                    type: "broadcast",
                    participants: ["agent-002", "agent-003", "agent-004"],
                },
                { type: "sync", participants: ["agent-002"] },
            ];
            // Act
            const results = await Promise.all(sessionTypes.map((config, i) => protocol.createMultimediaSession({
                type: config.type,
    ...
    describe("Multimedia Messaging", () => {
        ...
        it("should handle different message types", async () => {
            ...
            const messageTypes = [
                {
                    type: "stream_start",
                    payload: { contentType: "audio", encoding: "Opus" },
                },
                {
                    type: "stream_data",
                    payload: {
                ...
                {
                    type: "stream_end",
                    payload: { contentType: "control", encoding: "json" },
                },
                {
                    type: "sync_signal",
                    payload: { contentType: "control", encoding: "json" },
            ...
            // Act
            const results = await Promise.all(messageTypes.map((msgType, i) => protocol.sendMultimediaMessage({
                type: msgType.type,
    ...
    describe("Multimedia Streaming", () => {
        ...
        it("should start multimedia stream successfully", async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith("Starting multimedia stream", expect.objectContaining({
                sessionId,
        ...
        it("should handle different media types for streaming", async () => {
            // Arrange
            const mediaTypes = ["video", "audio", "mixed"];
            // Act
            const results = await Promise.all(mediaTypes.map((mediaType) => protocol.startMultimediaStream(sessionId, {
                sourceAgentId: "stream-initiator",
                targetAgents: ["stream-participant"],
                mediaType,
                quality: "medium",
    ...
    describe("Content Synchronization", () => {
        ...
        it("should synchronize content across agents", async () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith("Synchronizing multimedia content", expect.objectContaining({
                sessionId,
        ...
        it("should handle synchronization with different tolerances", async () => {
            ...
            // Act
            const results = await Promise.all(toleranceTests.map((tolerance, i) => protocol.synchronizeContent(sessionId, {
    ...
    describe("Protocol Performance Metrics", () => {
        ...
        it("should handle high-throughput scenarios", async () => {
            ...
            const concurrentSessions = Array.from({ length: 10 }, (_, i) => protocol.createMultimediaSession({
            ...
            expect(results.filter((r) => r.status === "fulfilled" && r.value.success)
...
//# sourceMappingURL=a2a-multimedia-protocol.test.js.map