...
describe("ResourceCoordinator", () => {
    ...
    describe("Resource Allocation", () => {
        ...
        it("should select optimal resource pools based on requirements", async () => {
            ...
            // Assert
            expect(result.success).toBe(true);
            expect(result.data.pools.some((pool) => pool.poolId.includes("gpu"))).toBe(true);
        });
    });
    describe("Resource Deallocation", () => {
        ...
        it("should return resources to available pool after deallocation", async () => {
            ...
            const initialAvailableMemory = poolsBefore.data.reduce((sum, pool) => sum + pool.available.memory, 0);
            // Act
            await resourceCoordinator.deallocateResources(allocationId);
            // Assert
            const poolsAfter = await resourceCoordinator.listPools();
            const finalAvailableMemory = poolsAfter.data.reduce((sum, pool) => sum + pool.available.memory, 0);
    ...
    describe("Performance and Scalability", () => {
        ...
        it("should handle concurrent allocation requests", async () => {
            ...
            const results = await Promise.allSettled(concurrentRequests.map((req) => resourceCoordinator.allocateResources(req)));
        ...
        it("should maintain performance under high load", async () => {
            ...
            // Act
            const startTime = Date.now();
            const results = await Promise.allSettled(loadTestRequests.map((req) => resourceCoordinator.allocateResources(req)));
            const endTime = Date.now();
            // Assert
            ...
            const successfulAllocations = results.filter((r) => r.status === "fulfilled" && r.value.success);
...
//# sourceMappingURL=resource-coordinator.test.js.map