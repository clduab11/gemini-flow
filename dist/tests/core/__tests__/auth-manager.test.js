...
describe("AuthenticationManager - Tier Detection", () => {
    ...
    describe("detectUltraTier", () => {
        it("should detect ultra tier with high confidence", async () => {
            const authManagerWithMocks = authManager;
            ...
            const result = await authManagerWithMocks.detectUltraTier("user@ultra-test.com", "ultra-test.com", { access_token: "test-token" });
        ...
        it("should not detect ultra tier with low confidence", async () => {
            ...
            authManagerWithMocks.checkCustomIntegrations = jest
                .fn()
                .mockResolvedValue(false);
            const result = await authManagerWithMocks.detectUltraTier("user@example.com", "example.com", { access_token: "test-token" });
    ...
    describe("detectEnterpriseTier", () => {
        it("should detect enterprise tier for known enterprise domains", async () => {
            ...
            authManagerWithMocks.checkEnterpriseScopes = jest
                .fn()
                .mockResolvedValue(true);
            const result = await authManagerWithMocks.detectEnterpriseTier("user@microsoft.com", "microsoft.com", { access_token: "test-token", scope: "admin.directory" });
        ...
        it("should not detect enterprise tier for regular domains", async () => {
            ...
            authManagerWithMocks.checkEnterpriseScopes = jest
                .fn()
                .mockResolvedValue(false);
            const result = await authManagerWithMocks.detectEnterpriseTier("user@gmail.com", "gmail.com", { access_token: "test-token" });
    ...
    describe("detectProTier", () => {
        it("should detect pro tier with valid subscription", async () => {
            ...
            const result = await authManagerWithMocks.detectProTier("user@example.com", "example.com");
    ...
    describe("caching behavior", () => {
        it("should cache tier detection results", async () => {
            ...
            expect(mockCache.set).toHaveBeenCalledWith("tier-detection:user@example.com", expect.any(Object), 86400);
        });
...
describe("AuthenticationManager - Integration", () => {
    ...
    describe("end-to-end tier detection", () => {
        it("should detect tier and create user profile with metadata", async () => {
            ...
            jest
                .spyOn(authManager, "authenticateUser")
