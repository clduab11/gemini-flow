...
describe("Smart Routing Integration Tests", () => {
    ...
    beforeEach(() => {
        ...
        jest
            .spyOn(orchestrator, "executeGeminiRequest")
    ...
    describe("End-to-End Routing Performance", () => {
        ...
        test("should maintain routing performance with complex contexts", async () => {
            ...
            const startTime = performance.now();
            const response = await orchestrator.orchestrate("Create a comprehensive ML pipeline solution", complexContext);
            const totalTime = performance.now() - startTime;
            expect(response.modelUsed).toBeDefined();
        ...
        test("should handle rapid successive requests efficiently", async () => {
            ...
            const startTime = performance.now();
            const promises = contexts.map((context) => orchestrator.orchestrate(`Prompt ${context.task}`, context));
    ...
    describe("Cache Integration", () => {
        ...
        test("should maintain cache performance under load", async () => {
            ...
            for (let i = 0; i < repeatRequests; i++) {
                const response = await orchestrator.orchestrate(`Unique prompt ${i % 10}`, {
                    ...baseContext,
                    task: `Task ${i % 10}`,
                });
                if (response.cached) {
    ...
    describe("Failover and Resilience", () => {
        ...
        test("should maintain performance during failover", async () => {
            ...
            const promises = Array(requests)
                .fill(null)
                .map((_, i) => orchestrator.orchestrate(`Resilience test ${i}`, context));
    ...
    describe("User Tier Compliance", () => {
        test("should respect tier restrictions in routing", async () => {
            const contexts = [
                { userTier: "free", expectedModel: "gemini-2.0-flash" },
                { userTier: "pro", expectedModelPrefix: "gemini-2.0-flash" },
                {
                    userTier: "enterprise",
                    expectedModelOption: [
                        "gemini-pro-vertex",
            ...
            for (const { userTier, expectedModel, expectedModelPrefix, expectedModelOption, } of contexts) {
        ...
        test("should handle tier upgrades appropriately", async () => {
            ...
            const response = await orchestrator.orchestrate("Complex task", baseContext);
    ...
    describe("Health Monitoring", () => {
        ...
        test("should detect unhealthy models", async () => {
            ...
            jest
                .spyOn(orchestrator, "executeWithModel")
        ...
        test("should maintain consistent performance over time", async () => {
            ...
            for (let batch = 0; batch < batches; batch++) {
                ...
                const promises = Array(batchSize)
                    .fill(null)
                    .map((_, i) => orchestrator.orchestrate(`Consistency test batch ${batch} item ${i}`, {
            ...
            const avgTimes = batchTimes.reduce((a, b) => a + b, 0) / batchTimes.length;
            const maxDeviation = Math.max(...batchTimes.map((t) => Math.abs(t - avgTimes)));
            expect(maxDeviation).toBeLessThan(avgTimes * 0.5); // Less than 50% deviation
        });
    });
});
//# sourceMappingURL=routing-integration.test.js.map