...
describe("Smart Routing Engine Performance Tests", () => {
    ...
    describe("Fallback Strategies", () => {
        test("should handle model unavailability with <75ms fallback", async () => {
            ...
            const startTime = performance.now();
            const decision = await router.selectFallbackModel("gemini-2.0-flash-thinking", context, mockModels, "Model unavailable");
        ...
        test("should gracefully degrade to lower tier models", async () => {
            ...
            const decision = await router.selectFallbackModel("gemini-pro-vertex", context, mockModels, "Service maintenance");
    ...
    describe("Comprehensive Statistics", () => {
        test("should provide detailed router statistics", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                router.recordPerformance(mockModels.get(Object.keys(Object.fromEntries(mockModels))[i % mockModels.size]).name, Math.random() * 1000 + 500, // latency
...
//# sourceMappingURL=smart-routing-performance.test.js.map