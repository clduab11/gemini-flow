...
global.testUtils = {
    ...
    expectToBeBetween: (actual, min, max) => {
        expect(actual).toBeGreaterThanOrEqual(min);
        expect(actual).toBeLessThanOrEqual(max);
    },
    expectArrayToContainObjectWithProperty: (array, property, value) => {
...
jest.mock('../../src/integrations/veo3/video-generation-pipeline', () => ({
    Veo3Integration: jest.fn().mockImplementation(() => ({
        ...
        generateVideo: jest.fn().mockImplementation((config) => Promise.resolve({
            success: true,
            data: {
...
jest.mock('../../src/streaming/enhanced-streaming-api', () => ({
    EnhancedStreamingAPI: jest.fn().mockImplementation(() => ({
        ...
        createVideoStream: jest.fn().mockImplementation((config) => Promise.resolve({
            success: true,
            data: {
                id: global.testUtils.generateRandomId(),
                status: 'active',
...
jest.mock('../../src/agentspace/core/AgentSpaceManager', () => ({
    AgentSpaceManager: jest.fn().mockImplementation(() => ({
        ...
        spawnAgent: jest.fn().mockImplementation((config) => Promise.resolve({
            success: true,
            data: {
                id: config.id,
                status: 'active',
