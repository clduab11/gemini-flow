...
describe("Byzantine Consensus Fault Injection Tests", () => {
    ...
    describe("Malicious Agent Attacks", () => {
        test("should detect and handle double voting", async () => {
            ...
            const behaviors = await maliciousDetection.analyzeBehavior(maliciousAgent, [], [vote1, vote2].map((v) => ({ ...v, id: "vote-id", signature: "sig" })));
        ...
        test("should handle conflicting messages from malicious agents", async () => {
            ...
            // Analyze behavior
            const behaviors = await maliciousDetection.analyzeBehavior(maliciousAgent, [message1, message2], []);
        ...
        test("should detect and mitigate spam flooding", async () => {
            ...
            const behaviors = await maliciousDetection.analyzeBehavior(spammerAgent, spamMessages, []);
        ...
        test("should handle collusion between multiple agents", async () => {
            ...
            for (const agentId of colludingAgents) {
                const behaviors = await maliciousDetection.analyzeBehavior(agentId, [], votes.filter((v) => v.voterId === agentId));
    ...
    describe("Performance Degradation Tests", () => {
        test("should handle high latency conditions", async () => {
            ...
            consensus["broadcastMessage"] = async function (message) {
    ...
    describe("Security Attack Scenarios", () => {
        test("should resist eclipse attacks", async () => {
            ...
            for (const maliciousId of maliciousAgents) {
                const behaviors = await maliciousDetection.analyzeBehavior(maliciousId, [
                    {
                        type: "prepare",
                        viewNumber: 1,
                        sequenceNumber: 1,
...
describe("Integration Fault Tests", () => {
    ...
    test("should maintain data integrity under stress", async () => {
        ...
        const results = await Promise.allSettled(operations.map((op) => stateMachine.executeOperation(op)));
