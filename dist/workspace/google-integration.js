...
export class GoogleWorkspaceIntegration extends EventEmitter {
    auth; // OAuth2Client when available
    drive;
    docs;
    sheets;
    slides;
    ...
    async initializeAuth(config) {
        ...
        this.auth = new googleApis.google.auth.OAuth2(config.clientId, config.clientSecret, config.redirectUri || "http://localhost:3000/callback");
     ...
     */
    async initializeServices(google) {
        try {
     ...
     */
    async analyzeSpreadsheet(spreadsheetId, range, analysisType) {
        try {
    ...
    async createPresentation(title, slides) {
        try {
            ...
            const contentRequests = slides.flatMap((slide, index) => this.createSlideContentRequests(`slide_${index}`, slide));
            await this.slides.presentations.batchUpdate({
                presentationId,
     ...
     */
    async exportDocument(fileId, mimeType) {
        try {
     ...
     */
    async watchDocument(fileId, callback) {
    ...
    createSlideContentRequests(slideId, slide) {
        const requests = [];
        if (slide.title) {
            requests.push({
                insertText: {
