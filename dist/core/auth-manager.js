...
export class AuthenticationManager extends EventEmitter {
    ...
    async initializeAuth() {
        try {
            ...
            if (this.config.clientId &&
                ...
                this.oauth2Client = new googleApis.google.auth.OAuth2(this.config.clientId, this.config.clientSecret, this.config.redirectUri || "http://localhost:3000/callback");
            }
    ...
    async authenticateUser(code) {
        ...
        try {
            ...
            const oauth2Tokens = {
                ...
                scope: tokens.scope
                    ? tokens.scope.split(" ")
                    : this.config.scopes || this.DEFAULT_SCOPES,
        ...
        try {
            ...
            if (timeUntilExpiry <= 0) {
                this.logger.debug("Token expired", {
                    userId,
                    expiredAt: new Date(storedTokens.expiresAt),
                });
                return {
                    valid: false,
                    expired: true,
     ...
     */
    scheduleTokenRefresh(userId, tokens) {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                if (!result.success) {
                    this.logger.warn("Auto token refresh failed", {
                        userId,
                        error: result.error,
                    });
    ...
    isRefreshTokenInvalid(error) {
        ...
        return invalidTokenIndicators.some((indicator) => errorMessage.includes(indicator));
    }
    /**
     ...
     */
    async needsTokenRefresh(userId) {
     ...
     */
    async detectUserTier(email, tokens) {
        if (!email) {
            return {
                tier: "free",
                method: "default",
        ...
        try {
            ...
            if (cachedResult) {
                this.logger.debug("Using cached tier detection", {
                    email,
     ...
     */
    async detectUltraTier(email, domain, tokens) {
        const features = [];
        let confidence = 0;
        try {
     ...
     */
    async detectEnterpriseTier(email, domain, tokens) {
        const features = [];
        let confidence = 0;
        try {
            ...
            if (tokens &&
                this.config.tierDetection?.enableWorkspaceIntegration !== false) {
     ...
     */
    async detectProTier(email, _domain) {
        const features = [];
        let confidence = 0;
        try {
     ...
     */
    async checkVertexAIAccess(tokens) {
        try {
            const googleApis = await safeImport("googleapis");
            ...
            const tempClient = new googleApis.google.auth.OAuth2(this.config.clientId, this.config.clientSecret);
            ...
            if (projects.data.projects && projects.data.projects.length > 0) {
                for (const project of projects.data.projects) {
                    try {
                        const serviceUsage = googleApis.google.serviceusage({
                            version: "v1",
                            auth: tempClient,
                        });
                        ...
                        const hasVertexAI = services.data.services?.some((service) => service.name?.includes("aiplatform.googleapis.com"));
                        if (hasVertexAI) {
                            return true;
                        }
                    }
     ...
     */
    async checkGoogleAIAdvanced(_email) {
     ...
     */
    async checkEnterpriseBilling(_email, _domain) {
     ...
     */
    async checkCustomIntegrations(_email) {
     ...
     * Check Google Workspace status
     */
    async checkGoogleWorkspace(email, tokens) {
        try {
            const googleApis = await safeImport("googleapis");
            if (!googleApis?.google) {
            ...
            const admin = googleApis.google.admin({
     ...
     * Check for enterprise OAuth scopes
     */
    async checkEnterpriseScopes(tokens) {
        ...
        const hasEnterpriseScope = enterpriseScopes.some((scope) => tokens.scope.includes(scope));
        return hasEnterpriseScope;
    }
    /**
     * Check for payment method (enhanced from existing stub)
     */
    async checkPaymentMethod(_email) {
     ...
     */
    async analyzeUsagePatterns(email) {
        try {
            const indicators = [];
     ...
     */
    async getUserPermissions(email, tier) {
     ...
     */
    async checkProSubscription(_email) {
            ...
            if (!tokenValidation.valid) {
                ...
                if (tokenValidation.expired) {
                    ...
                    if (storedTokens?.refreshToken) {
                        this.logger.debug("Attempting token refresh for expired session", {
                            userId,
                        });
                        const refreshResult = await this.refreshToken(userId);
                        if (refreshResult.success) {
                            this.logger.info("Session restored via token refresh", {
                                userId,
                            });
                        }
                        else if (refreshResult.requiresReauth) {
                            this.logger.warn("Session requires re-authentication", {
                                userId,
                            });
                            // Clean up session
                        ...
                        this.logger.warn("Session expired and no refresh token available", {
                            userId,
                        });
                        await this.cache.delete(`user:${userId}`);
                        this.emit("session_expired", userId);
     ...
     * Check quota usage
     */
    async checkQuota(userId, requestCount = 1) {
        const profile = await this.validateSession(userId);
        if (!profile) {
            return false;
        }
            ...
            if (storedTokens) {
                ...
                if (this.oauth2Client && storedTokens.accessToken) {
                    try {
                        ...
                        this.logger.debug("Token revocation at provider failed", {
                            userId,
                            error,
                        });
                    }
     ...
     */
    async determineUserTier(email, tokens) {
