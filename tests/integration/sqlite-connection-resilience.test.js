  ...
  describe('Auto-Reconnect Functionality', () => {
    test('should detect connection loss and auto-reconnect', async () => {
      ...
      const connection = await createResilientConnection(
        path.join(testDbDir, 'auto-reconnect-test.db'),
        {
          autoReconnect: true,
          maxRetries: 3,
          retryDelay: 50
    ...
    test('should implement exponential backoff for retry attempts', async () => {
      ...
      const connection = await createResilientConnection(
        path.join(testDbDir, 'backoff-test.db'),
        {
          autoReconnect: true,
          maxRetries: 4,
          retryDelay: 100,
    ...
    test('should fail after maximum retry attempts', async () => {
      ...
      const connection = await createResilientConnection(
        path.join(testDbDir, 'fail-test.db'),
        {
          autoReconnect: true,
          maxRetries: 2,
          retryDelay: 50
  ...
  describe('Transaction Coordination Across Failures', () => {
    ...
    test('should handle distributed transactions across multiple connections', async () => {
      ...
      try {
        ...
        await connections[0].query('INSERT INTO coordination_test VALUES (?, ?, ?, ?)', 
          ['agent-1', 'task-1', 'in_progress', JSON.stringify({ step: 1 })]);
        
        await connections[1].query('INSERT INTO coordination_test VALUES (?, ?, ?, ?)', 
          ['agent-2', 'task-1', 'coordinating', JSON.stringify({ step: 2 })]);
        
        await connections[2].query('INSERT INTO coordination_test VALUES (?, ?, ?, ?)', 
          ['agent-3', 'task-1', 'waiting', JSON.stringify({ step: 3 })]);
    ...
    test('should handle transaction recovery after reconnection', async () => {
      ...
      const connection = await createResilientConnection(
        path.join(testDbDir, 'transaction-recovery.db'),
        {
          autoReconnect: true,
          ...
          try {
            for (let j = 0; j < operationsPerConcurrent; j++) {
              const result = await conn.transaction(async (tx) => {
                ...
                // Update data
                const updateResult = await tx.query('UPDATE test_data SET data = ? WHERE data = ?', 
                  [`updated-${i}-${j}`, `concurrent-${i}-${j}`]);
