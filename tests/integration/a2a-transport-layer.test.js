...
import { A2ATransportLayer } from '../../src/protocols/a2a/core/a2a-transport-layer';
describe('A2A Transport Layer Integration', () => {
    ...
    describe('Initialization and Configuration', () => {
        it('should initialize with multiple transport configurations', async () => {
            ...
            await expect(newTransportLayer.initialize(mockConfigs)).resolves.not.toThrow();
        ...
        it('should validate transport configurations during initialization', async () => {
            const invalidConfigs = [
                ...
                { protocol: 'unsupported', host: 'test.com', port: 80 } // Unsupported protocol
            ];
            const newTransportLayer = new A2ATransportLayer();
    ...
    describe('Connection Management', () => {
        ...
        it('should handle connection failures gracefully', async () => {
            ...
            await expect(transportLayer.connect(testAgentId, invalidConfig)).rejects.toThrow(/Connection failed/);
        });
        it('should respect connection pool limits', async () => {
            const connections = [];
            // Try to create many connections for the same agent
            const connectionPromises = Array.from({ length: 10 }, (_, i) => transportLayer.connect(`${testAgentId}-${i}`, mockConfigs[0]));
        ...
        it('should emit connection events', async () => {
            ...
            expect(establishedSpy).toHaveBeenCalledWith(expect.objectContaining({
                id: connection.id,
                agentId: testAgentId
            }));
            await transportLayer.disconnect(connection.id);
        ...
        it('should reject connections with invalid authentication', async () => {
            ...
            await expect(transportLayer.connect(testAgentId, invalidAuthConfig)).rejects.toThrow(/authentication failed/i);
        });
    });
    describe('Message Routing', () => {
        ...
        it('should handle message timeouts', async () => {
            ...
            const timeoutConnection = await transportLayer.connect(`${testAgentId}-timeout`, shortTimeoutConfig);
            const slowMessage = {
                jsonrpc: '2.0',
            ...
            await expect(transportLayer.sendMessage(timeoutConnection.id, slowMessage)).rejects.toThrow(/timeout/i);
        ...
        it('should send notifications without expecting responses', async () => {
            ...
            await expect(transportLayer.sendNotification(connection.id, notification)).resolves.not.toThrow();
        ...
        it('should handle malformed messages gracefully', async () => {
            ...
            await expect(transportLayer.sendMessage(connection.id, malformedMessage)).resolves.toBeDefined();
        });
    ...
    describe('Broadcast and Multicast', () => {
        ...
        beforeEach(async () => {
            ...
            for (let i = 0; i < agentIds.length; i++) {
                const connection = await transportLayer.connect(agentIds[i], mockConfigs[i % mockConfigs.length]);
                connections.push(connection);
            }
        });
        afterEach(async () => {
        ...
        it('should exclude specified connections from broadcast', async () => {
            ...
            const responses = await transportLayer.broadcastMessage(broadcastMessage, [excludeConnectionId]);
    ...
    describe('Performance and Metrics', () => {
        ...
        beforeEach(async () => {
            testConnections = [];
            for (let i = 0; i < 3; i++) {
                const connection = await transportLayer.connect(`perf-agent-${i}`, mockConfigs[i % mockConfigs.length]);
    ...
    describe('Error Handling and Recovery', () => {
        it('should handle connection drops gracefully', async () => {
            ...
            await expect(transportLayer.sendMessage(connection.id, message)).rejects.toThrow(/Connection not found|not active/);
        });
        ...
        it('should handle transport layer shutdown gracefully', async () => {
            const connections = [];
            for (let i = 0; i < 3; i++) {
                const connection = await transportLayer.connect(`shutdown-test-${i}`, mockConfigs[i % mockConfigs.length]);
                connections.push(connection);
            }
        ...
        it('should emit error events for failed operations', async () => {
            ...
            await expect(transportLayer.connect('error-test-agent', invalidConfig)).rejects.toThrow();
        });
    });
});
//# sourceMappingURL=a2a-transport-layer.test.js.map