...
describe('ExecuteCommand', () => {
    ...
    describe('Framework Detection', () => {
        test('should detect FastAPI framework', async () => {
            ...
            const context = await executeCommand.analyzeExecutionContext();
    ...
    describe('Dependency Analysis', () => {
        test('should parse Python requirements.txt', async () => {
            ...
            const context = await executeCommand.analyzeExecutionContext(undefined, { framework: 'fastapi' });
        ...
        test('should parse package.json dependencies', async () => {
            ...
            const context = await executeCommand.analyzeExecutionContext(undefined, { framework: 'express' });
...
dependencies = [
    ...
    describe('File Scanning', () => {
        test('should scan Python files for FastAPI project', async () => {
            ...
            const context = await executeCommand.analyzeExecutionContext(undefined, { framework: 'fastapi' });
            const pyFiles = context.files.filter((f) => f.endsWith('.py'));
            expect(pyFiles.length).toBeGreaterThan(0);
            expect(pyFiles.some((f) => f.includes('main.py'))).toBe(true);
            expect(pyFiles.some((f) => f.includes('models.py'))).toBe(true);
            expect(pyFiles.some((f) => f.includes('users.py'))).toBe(true);
        });
        test('should scan JavaScript/TypeScript files for React project', async () => {
            ...
            const context = await executeCommand.analyzeExecutionContext(undefined, { framework: 'react' });
            const jsFiles = context.files.filter((f) => f.match(/\.(js|jsx|ts|tsx)$/));
            expect(jsFiles.length).toBeGreaterThan(0);
            expect(jsFiles.some((f) => f.includes('App.js'))).toBe(true);
            expect(jsFiles.some((f) => f.includes('Header.tsx'))).toBe(true);
        });
        test('should ignore node_modules and hidden directories', async () => {
            ...
            expect(context.files.some((f) => f.includes('.git'))).toBe(false);
        });
    });
    describe('Environment Setup', () => {
        ...
        test('should set Next.js environment variables', async () => {
            const context = await executeCommand.analyzeExecutionContext(undefined, { framework: 'nextjs' });
        ...
        test('should set Django environment variables', async () => {
            const context = await executeCommand.analyzeExecutionContext(undefined, { framework: 'django' });
    ...
    describe('Execution Commands', () => {
        test('should generate correct FastAPI execution command', () => {
            ...
            const command = executeCommand.getExecutionCommand(context);
    ...
    describe('Test Commands', () => {
        test('should generate correct pytest command', () => {
            ...
            const command = executeCommand.getTestCommand(context);
...
TOTAL                   40      2    95%
      `;
            const coverage = executeCommand.extractCoverage(output, 'pytest');
...
----------------------|---------|----------|---------|---------|-------------------
      `;
            const coverage = executeCommand.extractCoverage(output, 'jest');
            expect(coverage).toBe(88.24);
        });
        test('should return 0 for unknown test framework', () => {
            const output = 'Some test output';
            const coverage = executeCommand.extractCoverage(output, 'unknown');
    ...
    describe('File Type Detection', () => {
        test('should identify relevant Python files for FastAPI', () => {
            expect(executeCommand.isRelevantFile('main.py', '.py', 'fastapi')).toBe(true);
            expect(executeCommand.isRelevantFile('models.py', '.py', 'fastapi')).toBe(true);
            expect(executeCommand.isRelevantFile('requirements.txt', '.txt', 'fastapi')).toBe(true);
            expect(executeCommand.isRelevantFile('config.yaml', '.yaml', 'fastapi')).toBe(true);
        });
        test('should identify relevant JavaScript/TypeScript files for React', () => {
            expect(executeCommand.isRelevantFile('App.js', '.js', 'react')).toBe(true);
            expect(executeCommand.isRelevantFile('Component.jsx', '.jsx', 'react')).toBe(true);
            expect(executeCommand.isRelevantFile('App.ts', '.ts', 'react')).toBe(true);
            expect(executeCommand.isRelevantFile('Component.tsx', '.tsx', 'react')).toBe(true);
            expect(executeCommand.isRelevantFile('package.json', '.json', 'react')).toBe(true);
        });
        test('should reject irrelevant files', () => {
            expect(executeCommand.isRelevantFile('data.csv', '.csv', 'fastapi')).toBe(false);
            expect(executeCommand.isRelevantFile('image.png', '.png', 'react')).toBe(false);
        });
    });
...
//# sourceMappingURL=execute-command.test.js.map