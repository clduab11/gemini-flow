...
class AgentCardSystem {
    ...
    async initialize() { }
    async shutdown() { }
    async registerAgent(agentCard, ttl) { return {}; }
    async unregisterAgent(agentId) { return true; }
    async updateAgentCard(agentCard) { return true; }
    async getAgentCard(agentId) { return null; }
    async discoverAgents(request) { return {}; }
    async findAgentsByCapability(capabilityName, version) { return []; }
    async findAgentsByType(agentType) { return []; }
    async findAgentsByService(serviceName) { return []; }
    async refreshAgentStatus(agentId) { return true; }
    async getSystemMetrics() { return {}; }
    getRegisteredAgents() { return new Map(); }
    getDiscoveryMetrics() { return {}; }
}
describe('AgentCardSystem', () => {
    ...
    describe('Advanced Filtering', () => {
        ...
        it('should filter by resource requirements', async () => {
            ...
            const response = await agentCardSystem.discoverAgents(discoveryRequest);
            response.result.agents.forEach(agent => {
                const hasGpuCapability = agent.capabilities.some(cap => cap.resources?.specialized?.includes('gpu-acceleration'));
...
//# sourceMappingURL=agent-card-system.test.js.map