...
import type { AdapterConfig, ModelRequest, ModelResponse, RequestContext } from '../../../src/adapters/base-model-adapter';
  ...
  protected handleError(error: any, request: ModelRequest): any {
    if (error instanceof Error) {
      return this.createError(
        ...
        500,
        true,
        { originalError: error, requestId: request.context?.requestId }
      );
    }
    return this.createError(
...
describe('BaseModelAdapter', () => {
  ...
  describe('initialization', () => {
    ...
    it('should throw error when calling generate before initialization', async () => {
      const request: ModelRequest = { prompt: 'test' };
  ...
  describe('request ID generation', () => {
    it('should generate unique request IDs', () => {
      const id1 = (adapter as any).generateRequestId();
      const id2 = (adapter as any).generateRequestId();
  ...
  describe('context creation', () => {
    it('should create context with defaults when no context provided', () => {
      const context = (adapter as any).ensureRequestId();
    ...
    it('should add requestId when missing from existing context', () => {
      ...
      const context = (adapter as any).ensureRequestId(partial);
  ...
  describe('error handling', () => {
    it('should create adapter errors with proper structure', () => {
      const error = (adapter as any).createError('Test error', 'TEST_ERROR', 500, true);
    ...
    it('should create errors with default values', () => {
      const error = (adapter as any).createError('Simple error', 'SIMPLE_ERROR');
      expect(error).toBeInstanceOf(Error);
    ...
    it('should create errors with metadata', () => {
      ...
      const error = (adapter as any).createError('Test error', 'TEST_ERROR', 500, true, metadata);
      expect(error.metadata).toEqual(metadata);
    });

    it('should handle different error types', () => {
      const errorObj = new Error('Original error');
      const handled = (adapter as any).handleError(errorObj, {} as ModelRequest);
    ...
    it('should handle unknown error types', () => {
      const handled = (adapter as any).handleError('string error', {} as ModelRequest);
      expect(handled).toBeInstanceOf(Error);
  ...
  describe('performance logging', () => {
    it('should log performance metrics', () => {
      const logSpy = jest.spyOn((adapter as any).logger, 'info');
      (adapter as any).logPerformance('test-operation', 150, true);
    ...
    it('should log performance metrics with metadata', () => {
      ...
      (adapter as any).logPerformance('test-operation', 150, true, metadata);
  ...
  describe('cost calculation', () => {
    it('should calculate cost based on token usage', () => {
      ...
      const cost = (adapter as any).calculateCost(usage, costPerToken);
