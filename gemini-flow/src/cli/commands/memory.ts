...
export class MemoryCommand extends Command {
  ...
  constructor() {
    ...
    this.description('Manage persistent memory across sessions')
      .alias('mem');
  ...
  private async store(key: string, value: string, options: any): Promise<void> {
    try {
  ...
  private async query(pattern: string, options: any): Promise<void> {
    ...
    try {
      ...
      // Simulated results
      const results = [
        { key: 'project/config', value: '{"name":"gemini-flow","version":"2.0.0"}', namespace: 'default' },
        { key: 'agent/last-task', value: 'Code review completed', namespace: 'swarm' },
        { key: 'performance/metrics', value: '{"latency":127,"throughput":2341}', namespace: 'monitoring' }
      ];

      const filtered = pattern === '*' ? results : 
  ...
  private async export(filename?: string, options?: any): Promise<void> {
  ...
  private async import(filename: string, options: any): Promise<void> {
    ...
    try {
      ...
      spinner.text = options.merge ? 'Merging with existing...' : 'Importing entries...';
  ...
  private async clear(options: any): Promise<void> {
    if (!options.force) {
    ...
    try {
      ...
      console.log(chalk.green('\n✅ Memory cleared:'));
      console.log(chalk.gray('  Scope:'), options.all ? 'All namespaces' : 
